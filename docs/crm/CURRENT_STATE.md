...
## Revalidação autónoma em 2026-07-22T00:28:29Z

A retoma começou no `HEAD` limpo e sincronizado `f9a01cbdb3b01d8c0344dc9099741b1c2249a781`, na branch esperada. O plano canónico, este documento, commits, testes, migrations, PR, processos, staging, produção, backup, rollback e telemetria foram reconciliados antes de agir. Não existia trabalho staged, unstaged ou untracked. Não existiam processos locais ou remotos de worker CRM, reconciler ou outbox publisher; os muitos containers locais preexistentes foram preservados como trabalho desconhecido.

Em PostgreSQL 16 descartável e exclusivo, migrado de raiz, a suite Python segura completa passou com `1200 passed, 1 skipped` e exit explícito `0`. O frontend Node passou `37/37`; `compileall`, `node --check` e `git diff --check` passaram. Um segundo PostgreSQL 16 exclusivo passou o lifecycle `base -> 0013 -> 0011 -> 0013 -> base -> 0013`, `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. Um dump custom-format local foi restore-verificado com schema `0013`, 15 tabelas obrigatórias, zero workspaces e zero violações. Ruff lint e Gitleaks passaram; o format check global do delta continua a identificar sete ficheiros históricos já documentados, sem alteração funcional nesta retoma.

O staging no proxy/TLS final está agora na imagem exata `crm-staging:f9a01cb`. Aplicação e PostgreSQL 16 estão `healthy`, com zero restarts, schema `0013`, um workspace, 46 contas, 65 leads e 44 propostas. Contas e Propostas leem PostgreSQL e o command writer de staging usa PostgreSQL; projeção Sheets e Agent ingress permanecem desligados. `/up` devolveu `200`; Leads, Contas, Propostas, Inteligência, Operações e APIs v1 autenticadas devolveram `200` com `Cache-Control: no-store`; pedidos sem credenciais devolveram `401`. Existem zero violações da invariante de Account, zero outbox pendente, zero eventos failed/dead-letter e zero marcadores de erro nos logs das duas horas observadas. O timer de backup de staging permanece enabled/active e não há workers outbound ativos.

Foi criado um dump custom-format pós-deploy e copiado off-host para `/Users/max/.hermes/profiles/marketing-max/backups/crm/staging/crm-staging-20260722T002721Z-post-f9a01cb.dump`, mode `0600`, 200.589 bytes e SHA-256 `eba2fe1e3a3f83c3d6f96c9056f7e14d11f366f306f262a369a1267f93d73986`. O arquivo exato foi restore-verificado num PostgreSQL 16 separado com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. O rollback imediato de staging continua preservado no container/imagem anterior `cab1417`.

Produção permaneceu inalterada no build pré-revamp `7622a2b2b8d5e0790858208b2c3a1f119edb7328`: `/` e `/up` devolveram `200`, `/contas` devolveu `404` e `/api/stats` devolveu `200`. Não existe recurso PostgreSQL/backup CRM de produção nem configuração canónica de principal observável. A telemetria estruturada das últimas 48 horas, excluindo `curl`, voltou a provar consumidores v0 ativos: `/api/stats` 7 pedidos, `/api/outreach-followups` 4, `/api/email-followups` 4 e `/api/proposal-followups` 4, com pedidos até `2026-07-22T00:07:17Z`.

Consequentemente, os gates de produção continuam materialmente fechados: `parity=false`, conflitos reais sem resolução ou aceitação, amostra sem validação do owner, política oficial de `Won` e retenção/scopes por decidir, mapping final de principal/papel/workspace ausente, sem PostgreSQL de produção isolado com backup automático off-host e restore real, e sem cutover/soak de produção. A Tarefa 19 exige depois dois releases estáveis pós-cutover, ausência comprovada de consumidores v0, export e aceitação dos stakeholders. Fazer cutover, retirar o legado ou criar `.hermes/crm-revamp-complete.json` neste estado seria contrário aos gates técnicos e de dados do plano; nenhuma dessas ações foi executada.

---

## Verificação do candidato e staging em 2026-07-22T01:54:56Z

A retoma partiu do `HEAD` limpo e sincronizado `6243331c4bbe5b69a3b0399d30ce85eba75235ec`, na branch esperada. O plano canónico, commits, testes, migrations, documentação, PR, processos, containers, staging, produção, backups, rollback e telemetria foram reinspecionados. Não existia trabalho staged, unstaged ou untracked nem processos locais ou remotos de worker CRM, reconciler ou outbox publisher. Os muitos containers locais preexistentes foram preservados como trabalho desconhecido; apenas os três recursos descartáveis criados por esta verificação foram removidos no fim.

Num PostgreSQL 16 descartável e exclusivo, migrado de raiz, a suite Python segura completa passou com `1200 passed, 1 skipped`, uma `StarletteDeprecationWarning` de dependência e exit explícito `0`. O frontend Node passou `37/37`. Um segundo PostgreSQL 16 exclusivo passou o lifecycle `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. Um dump custom-format local foi restaurado e validado com schema `0013`, 15 tabelas obrigatórias, zero workspaces e zero violações. Ruff no delta Python, format check do delta funcional mais recente, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 150 commits e cerca de 2,10 MB sem leaks. As portas descartáveis `55970`, `55971` e `55972` ficaram livres.

O staging no proxy/TLS final já servia a imagem exata `crm-staging:6243331`, com aplicação e PostgreSQL `healthy`, zero restarts e schema `0013`. A base mantém um workspace, 46 contas, 65 leads e 44 propostas; existem zero Leads em fases que exigem Account sem associação, zero outbox rows e zero eventos `failed`/`dead_letter`. Leads, Contas, Propostas, Inteligência, Operações e as APIs protegidas devolveram `200` autenticado com `Cache-Control: no-store`; `/up` devolveu `200`. A página Operações passou browser real a 320 px sem overflow, erros de consola ou pedidos falhados. Um soak read-only adicional executou 90 pedidos com zero falhas, p95 de 79,5 ms, máximo de 88,8 ms e zero marcadores de erro nos logs. O timer de backup de staging continua `enabled`/`active`, não existem processos outbound e o rollback imediato permanece no container/imagem anterior `crm-staging:f9a01cb`.

O dump pós-deploy `crm-staging-20260722T012946Z.dump` foi copiado off-host para `/Users/max/.hermes/profiles/marketing-max/backups/crm/staging/crm-staging-20260722T012946Z-post-6243331.dump`, com mode `0600`, 200.589 bytes e SHA-256 `f7469f5b277870cd91671ec26c3a9c213e50513601902d564e8c2c0bffcd0e0d`. Os hashes remoto e local coincidiram e o arquivo exato foi restore-verificado num PostgreSQL 16 separado com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações.

Produção permaneceu inalterada e saudável no build pré-revamp `7622a2b2b8d5e0790858208b2c3a1f119edb7328`. Não existe container PostgreSQL CRM de produção nem timer de backup além do staging. A telemetria estruturada das últimas 48 horas, excluindo `curl`, continua a provar consumidores v0 ativos: `/api/stats` 7 pedidos, `/api/outreach-followups` 4, `/api/email-followups` 4 e `/api/proposal-followups` 4, com pedidos até `2026-07-22T00:07:17Z`.

A implementação local e o staging técnico estão verdes, mas a conclusão global continua bloqueada por gates que a autorização autónoma não transforma em evidência: comparação real `parity=false`; conflitos sem resolução ou aceitação; amostra sem validação do owner; política oficial de `Won`, retenção e scopes por decidir; mapping final de principal/papel/workspace de produção ausente; sem PostgreSQL CRM de produção com backup automático off-host e restore real; sem cutover ou soak de produção. A Tarefa 19 exige ainda dois releases estáveis pós-cutover, ausência comprovada de consumidores v0, export e aceitação dos stakeholders. O branch não foi merged, produção não foi alterada e `.hermes/crm-revamp-complete.json` não foi criado.

---

## Backfill operacional seguro de staging em 2026-07-22T02:53:08Z

A retoma partiu do `HEAD` limpo e sincronizado `935237f267b80928392a8305b57a7d8e585e654d`. A inspeção confirmou branch e PR corretos, ausência de trabalho staged/unstaged/untracked, staging na imagem exata `crm-staging:6243331`, PostgreSQL 16 healthy em `0013`, produção ainda no build pré-revamp `7622a2b2b8d5e0790858208b2c3a1f119edb7328` e zero workers, reconcilers ou outbox publishers no host. Recursos locais preexistentes e desconhecidos foram preservados.

Antes de qualquer write foi criado o backup custom-format `crm-staging-20260722T024957Z-pre-operations-backfill.dump`, copiado off-host com mode `0600`, 200.589 bytes e SHA-256 `b3381bb10812a30b4f980cf0f717817026e71f141374ed0df3791767d6ca55a1`. Os hashes remoto/local coincidiram e o arquivo exato foi restore-verificado num PostgreSQL 16 descartável: schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações.

O backfill operacional consumiu exclusivamente a snapshot imutável preservada, sem tocar na Sheet, e foi executado duas vezes contra staging. A primeira execução criou 21 tarefas e 86 atividades conservadoras; a segunda criou zero registos e devolveu 107 replay no-op. Permaneceram 387 conflitos fail-closed: 200 identidades sem Lead canónico, 61 sequências de outreach ambíguas, 53 tipos de último contacto desconhecidos, 25 sem tipo, 21 identidades estáveis ausentes, 12 grupos duplicados, 9 sem data de dashboard, 5 tarefas em fase terminal e 1 sequência terminal. Nenhum conflito foi adivinhado ou sobrescrito.

Depois do apply, staging manteve zero violações da invariante de Account, zero outbox rows e zero eventos `failed`/`dead_letter`. `/up`, Leads, o resumo/lista do pipeline e Operações devolveram `200` através do proxy/TLS autenticado; os logs desde o apply continham zero marcadores de erro. Os inputs temporários foram removidos do host e do container. O backup pós-apply `crm-staging-20260722T025211Z-post-operations-backfill.dump` foi copiado off-host com mode `0600`, 236.968 bytes e SHA-256 `9509c69a6a9af3bbfb0d062fe283ab62e07b13d50c58f04c6e57801ddbb5055d`; os hashes coincidiram e o arquivo exato foi restore-verificado com schema `0013`, um workspace e zero violações. O PostgreSQL descartável desta operação foi removido e a porta `55980` ficou livre.

Esta execução fecha o apply/replay seguro dos artefactos operacionais inequivocamente associados, mas não fecha a paridade de dados: linhas sem fase canónica ou identidade segura continuam deliberadamente fora do modelo, e a comparação permanece `parity=false`. Produção continua sem PostgreSQL CRM, mapping canónico ou backup CRM e os contratos v0 ainda têm consumidores observados nas últimas 48 horas. Continuam em falta validação da amostra pelo owner, decisões oficiais de `Won`/retenção/scopes, cutover/soak de produção, dois releases pós-cutover e ausência comprovada de consumidores v0. O sentinel permanece proibido.

### Gate exato pós-commit em 2026-07-22T03:03:21Z

O `HEAD` publicado `61c2b954c9bc4b624a74118fcca0dfc7a7ac26bd` foi verificado numa instância PostgreSQL 16 descartável e exclusiva, migrada de raiz até `0013`: a suite Python completa passou com `1200 passed, 1 skipped`, uma warning de depreciação da dependência Starlette e exit explícito `0`; o frontend Node passou `37/37` com exit explícito `0`. Uma tentativa inicial de lifecycle na base já usada pela suite falhou corretamente porque factos estruturados de transição impedem o downgrade destrutivo de `0012`. O lifecycle autoritativo foi repetido numa segunda base vazia e exclusiva: `base -> 0013 -> 0011 -> 0013 -> base -> 0013`, seguido de `alembic current = 0013 (head)` e `alembic check = No new upgrade operations detected`; todos os seis comandos tiveram exit `0`. Os dois containers foram removidos, as portas `55981` e `55982` ficaram livres e a worktree permaneceu limpa.

---

## Retoma autónoma em 2026-07-22T03:54:04Z

A retoma partiu do `HEAD` limpo e sincronizado `420a3a090c297debed19336395edf08c5394703c`, na branch esperada. O plano canónico, commits, testes, migrations, documentação, PR, processos, staging, produção, backups, rollback e telemetria foram reconciliados antes de agir. Não existia trabalho staged, unstaged ou untracked, nem processos locais ou remotos de worker CRM, reconciler ou outbox publisher. Os muitos containers locais preexistentes foram preservados como trabalho desconhecido; apenas os dois PostgreSQL descartáveis exclusivos criados nesta retoma foram removidos pelos respetivos traps.

Num PostgreSQL 16 descartável migrado de raiz, a suite Python segura completa passou com `1200 passed, 1 skipped`, uma warning de depreciação da dependência Starlette e exit explícito `0`. O frontend Node passou `37/37`. Numa segunda base vazia e exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)`, `alembic check` não detetou operações pendentes e um dump custom-format foi restore-verificado com schema `0013`, 15 tabelas obrigatórias, zero workspaces e zero violações. Ruff no delta Python, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 153 commits e cerca de 2,11 MB sem leaks.

O staging no proxy/TLS final permanece na imagem funcional `crm-staging:6243331`, com aplicação e PostgreSQL 16 `healthy`, zero restarts e schema `0013`. A base mantém um workspace, 46 contas, 46 contactos, 65 leads, 44 propostas, 44 versões, 21 tarefas e 151 atividades. Existem zero Leads em fases que exigem Account sem associação, zero outbox rows e zero eventos `failed`/`dead_letter`. `/up` devolveu `200`; pedidos sem credenciais a Leads, Contas, Propostas, Inteligência, Operações e APIs ricas devolveram `401`; os mesmos oito contratos devolveram `200` autenticado. O timer de backup está `enabled`/`active`, o último serviço terminou com sucesso e os logs das duas horas observadas continham zero marcadores de erro. O backup off-host pós-backfill operacional foi novamente verificado nesta retoma: mode `0600`, 236.968 bytes, SHA-256 `9509c69a6a9af3bbfb0d062fe283ab62e07b13d50c58f04c6e57801ddbb5055d`; o arquivo exato restaurou em PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Esta retoma não alterou staging.

Produção permaneceu inalterada no container saudável `leads-dashboard-web-7622a2b2b8d5e0790858208b2c3a1f119edb7328`, com zero restarts. `/` e `/up` devolveram `200`, as novas páginas/APIs devolveram `404` e `/api/stats` devolveu `200`. Não existe PostgreSQL, mapping canónico ou timer de backup CRM de produção; o único timer CRM observado é o de staging. A telemetria estruturada das últimas 48 horas, excluindo `curl`, confirmou consumidores v0 ativos: `/api/stats` 7 pedidos e quatro pedidos em cada um de `/api/outreach-followups`, `/api/email-followups` e `/api/proposal-followups`, com tráfego até `2026-07-22T00:07:17Z`.

A implementação e o staging técnico continuam verdes, mas os gates globais impedem com razão o cutover: comparação real `parity=false`; conflitos sem resolução ou aceitação; amostra sem validação do owner; política oficial de `Won`, retenção e scopes por decidir; mapping final de principal/papel/workspace de produção ausente; sem PostgreSQL CRM de produção com backup automático off-host e restore do arquivo real; sem cutover ou soak de produção. A Tarefa 19 exige ainda dois releases estáveis pós-cutover, ausência comprovada de consumidores v0, export e aceitação dos stakeholders. A autorização autónoma remove pausas de execução, mas não transforma estes gates de dados, humanos e temporais em evidência. Produção não foi alterada e `.hermes/crm-revamp-complete.json` não foi criado.

---

## Retoma autónoma em 2026-07-22T05:56:08Z

A retoma começou no `HEAD` limpo e sincronizado `41ef68f28e604d7ae94533bdcab9665c7361b98f`, na branch esperada. O plano canónico, este documento, commits, testes, migrations, PR, processos, staging, produção, backups, rollback e telemetria foram reinspecionados antes de agir. Não existia trabalho staged, unstaged ou untracked, nem processos locais ou remotos de worker CRM, reconciler ou outbox publisher. Os recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável e exclusivo, migrado de raiz, a suite Python segura completa passou com `1200 passed, 1 skipped`, uma warning de depreciação da dependência Starlette e exit explícito `0`. O frontend Node passou `37/37`. Numa segunda base vazia e exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. Um dump custom-format local foi restore-verificado com schema `0013`, 15 tabelas obrigatórias, zero workspaces e zero violações. Ruff no delta Python, `compileall`, `node --check` e `git diff --check` passaram. Os containers e ficheiros descartáveis criados nesta retoma foram removidos.

O staging no proxy/TLS final permanece na imagem funcional `crm-staging:6243331`, com aplicação e PostgreSQL 16 `healthy`, zero restarts e schema `0013`. A base mantém um workspace, 46 contas, 46 contactos, 65 leads, 44 propostas, 44 versões, 21 tarefas e 151 atividades; existem zero violações da invariante de Account, zero outbox pendente e zero eventos `failed`/`dead_letter`. Leads, Contas, Propostas, Inteligência, Operações e as APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`; o timer de backup está `enabled`/`active` e os logs das duas horas observadas continham zero marcadores de erro. O backup off-host pós-backfill operacional conserva mode `0600`, 236.968 bytes e SHA-256 `9509c69a6a9af3bbfb0d062fe283ab62e07b13d50c58f04c6e57801ddbb5055d`; o arquivo exato foi novamente restore-verificado em PostgreSQL 16 com schema `0013`, um workspace e zero violações. As imagens `f9a01cb`, `cab1417` e `6243331` permanecem disponíveis para rollback.

Produção permaneceu inalterada e saudável no build pré-revamp: `/` e `/up` devolveram `200`, `/contas` devolveu `404` e os contratos v0 acompanhados continuaram disponíveis. Nas últimas 48 horas, excluindo `curl`, a telemetria estruturada registou sete pedidos `2xx` em `/api/stats` e quatro em cada um de `/api/outreach-followups`, `/api/email-followups` e `/api/proposal-followups`, com tráfego até `2026-07-22T00:07:17Z`. O PR `#1` continua draft, mergeable e `CLEAN`, no SHA exato da branch, sem reviews, checks ou environments GitHub.

Não foi encontrada nova lacuna técnica que pudesse ser fechada sem violar gates. A conclusão global continua bloqueada por `parity=false`, conflitos sem resolução ou aceitação, amostra sem validação do owner, políticas oficiais de `Won`/retenção/scopes por decidir, mapping final de produção ausente, inexistência de PostgreSQL CRM de produção com backup automático off-host e restore real, cutover/soak de produção e, depois, dois releases estáveis sem consumidores v0 e aceitação dos stakeholders. Não houve deploy ou migração de produção, ativação de outbound, retirada do legado ou criação de `.hermes/crm-revamp-complete.json`.

---

## Verificação e rollout de staging do candidato `d061ce1` em 2026-07-22T08:00:30Z

A retoma começou no `HEAD` limpo e sincronizado `d061ce1dfc15fc6d3835b3d2913d5b4036166626`, na branch esperada. Foram preservados todos os recursos preexistentes desconhecidos; não existiam alterações staged, unstaged ou untracked nem processos CRM de worker, reconciler, outbox publisher ou jobs outbound ativos. Os dois commits posteriores ao último gate documentado fecham a probabilidade inequívoca no perfil de Account e impedem que o downgrade de `0010` destrua cidades canónicas preenchidas.

Num PostgreSQL 16 descartável e exclusivo, migrado de raiz, a regressão focada passou com `17 passed`; a suite Python segura completa passou com `1202 passed, 1 skipped`, uma warning de depreciação da dependência Starlette e exit explícito `0`. O frontend Node passou `37/37`. Numa base vazia exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. Um dump custom-format local foi restore-verificado com schema `0013`, 15 tabelas obrigatórias, zero workspaces e zero violações. Ruff no delta Python, `compileall`, `node --check` e `git diff --check` passaram.

Antes do rollout foi criado o backup custom-format de staging `crm-staging-20260722T075345Z-pre-d061ce1.dump`, copiado off-host para `/Users/max/.hermes/profiles/marketing-max/backups/crm/staging/`, mode `0600`, 236.968 bytes e SHA-256 `12a57252356f98cc80719df2c6f15c9b63d9cece65c6bca72fa018611ed8348f`. Os hashes remoto/local coincidiram e o arquivo exato foi restore-verificado em PostgreSQL 16 com schema `0013`, um workspace e zero violações.

O staging no proxy/TLS final foi atualizado para a imagem Linux AMD64 `crm-staging:d061ce1`, revisão exata `d061ce1dfc15fc6d3835b3d2913d5b4036166626`. A aplicação e o PostgreSQL permaneceram `healthy`, com zero restarts, schema `0013`, um workspace, 46 Accounts, 65 Leads, 44 Proposals, 21 Tasks, 151 Activities e zero Outbox rows. O command writer permanece PostgreSQL apenas em staging; projeção Sheets e Agent ingress permanecem desligados. `/up` devolveu `200`, pedidos sem credenciais a conteúdo protegido devolveram `401` e o Agent ingress desativado devolveu `404`. O browser autenticado carregou Leads, Contas, Propostas, Inteligência e Operações a 320×844 com `200`, zero overflow, zero erros de consola e zero requests falhados. O soak read-only executou `90/90` pedidos com sucesso, p95 `79,7 ms`, máximo `104,1 ms` e zero marcadores de erro nos logs. O rollback imediato foi preservado em `crm-staging-web-prev-6243331`/`crm-staging:6243331`.

O backup pós-rollout `crm-staging-20260722T075756Z-post-d061ce1.dump` foi copiado off-host com mode `0600`, 236.968 bytes e SHA-256 `4103bcdc48f65e493187f780467f3c6a016713253f0067e1b4a1fcb5834b4309`. Os hashes coincidiram e o arquivo exato foi restore-verificado com schema `0013`, um workspace e zero violações.

A comparação read-only repetida contra a snapshot imutável continua fail-closed: `parity=false`, 39 conflitos, 12 grupos duplicados, 66 imports esperados, 65 Leads e 46 Accounts correspondentes, um Lead e uma Account em falta, zero mismatches de fase/associação/estado/campos e 1.118 fases vazias ou não mapeadas. Estes casos não foram auto-fundidos nem aceites artificialmente.
...
## Revalidação autónoma em 2026-07-22T09:59:48Z

A execução partiu do `HEAD` limpo e sincronizado `6e50f5c82175789b120e627c4aa6d81f0a318006`, na branch esperada. O plano canónico, commits, suite, migrations, revisão de dados, staging, produção, backup, rollback e processos outbound foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável e exclusivo, migrado de raiz, a suite Python segura completa passou com `1202 passed, 1 skipped`, uma warning de depreciação da dependência Starlette e exit `0`; o frontend Node passou `37/37`. Numa segunda base vazia exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. Um dump custom-format local foi restaurado e validado com schema `0013`, 15 tabelas obrigatórias, zero workspaces e zero violações. O backup off-host pós-`d061ce1` foi novamente restore-verificado em PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff no delta Python, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 160 commits e cerca de 2,13 MB sem leaks. Os recursos descartáveis criados nesta execução foram removidos.

O staging continua na imagem funcional exata `crm-staging:d061ce1`; aplicação e PostgreSQL estão `healthy`, com zero restarts, schema `0013`, um workspace, 46 Accounts, 46 Contacts, 65 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks e 151 Activities. Existem zero violações da invariante de Account, zero Outbox rows e zero eventos `failed`/`dead_letter`. Leads, Contas, Propostas, Inteligência, Operações e APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`; o soak read-only executou 90 pedidos com zero falhas, p95 `183,1 ms`, máximo `318,9 ms` e zero marcadores de erro. O timer de backup permanece `enabled`/`active`, o último serviço terminou com sucesso e não existem workers, reconcilers ou publishers outbound.

A revisão agregada dos artefactos privados confirmou que os restantes conflitos não têm resolução automática segura: 21 linhas sem identidade estável, 12 grupos de identidade duplicada, uma identidade por resolver e quatro propostas sem Account correspondente. A evidência histórica prova rank 40 para 8 de 18 terminais, mantendo 10 sem prova; nenhuma destas classificações autoriza merge por nome, invenção de identidade ou certificação da verdade comercial. O pacote de 46 Accounts e 44 Proposals continua a exigir validação do owner.

Produção permaneceu inalterada e saudável no build pré-revamp, sem PostgreSQL CRM de produção ou mapping canónico. A telemetria normalizada das últimas 48 horas, limitada a GET `2xx`, excluindo `curl` e usando o path estruturado do proxy, observou 44 pedidos v0: `/api/stats` 14, `/api/outreach-followups` 12, `/api/email-followups` 8 e `/api/proposal-followups` 10, com tráfego até `2026-07-22T09:43:47Z`. Remover os contratos agora quebraria consumidores observados.

Não foi encontrada lacuna técnica determinística adicional. Permanecem fechados os gates explícitos de paridade/aceitação dos conflitos, validação comercial da amostra, política oficial de `Won`, retenção/scopes, mapping final de produção, PostgreSQL CRM de produção com backup automático off-host e restore real, cutover/soak e dois releases estáveis sem consumidores v0. Não houve merge, deploy/migração de produção, ativação de outbound, retirada do legado ou criação de `.hermes/crm-revamp-complete.json`.

---

## Retoma autónoma em 2026-07-22T10:26:24Z

A execução partiu do `HEAD` limpo e sincronizado `373dfc9da9031c299ead5632dba519b0ba9da5cf`, na branch esperada. O plano canónico, `CURRENT_STATE.md`, commits, staged/unstaged work, suite, migrations, PR, processos, staging, produção, backup, rollback e telemetria foram reinspecionados. Não existia trabalho local por preservar, nem processos locais ou remotos de worker CRM, reconciler ou outbox publisher. O sentinel permanecia ausente.

Num PostgreSQL 16 descartável exclusivo, explicitamente marcado para testes e removido pelo trap, a migration de raiz chegou a `0013` e a suite Python segura completa passou com `1202 passed, 1 skipped`, uma warning de depreciação da dependência Starlette e exit `0`. O frontend Node passou `37/37`. Numa segunda base vazia exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)`, `alembic check` não detetou operações pendentes e um dump custom-format foi restore-verificado com schema `0013`, 15 tabelas obrigatórias, zero workspaces e zero violações. Ruff no delta Python, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 161 commits e cerca de 2,14 MB sem leaks.

O PR `#1` aponta para o SHA exato da branch, permanece draft, mergeable e `CLEAN`, sem reviews, checks ou environments GitHub. O staging continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL estão `healthy`, com zero restarts, schema `0013`, um workspace, 46 Accounts, 46 Contacts, 65 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks e 151 Activities. Existem zero violações da invariante de Account, zero Outbox rows e zero eventos `failed`/`dead_letter`. `/up` devolveu `200`; conteúdo protegido sem credenciais devolveu `401`; Leads, Contas, Propostas, Inteligência, Operações e APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`. O timer de backup permanece `enabled`/`active`, o último serviço terminou com sucesso e não existem publishers outbound.

Produção permaneceu inalterada no build pré-revamp, com `/up=200`, `/contas=404` e `/api/stats=200`. A telemetria estruturada das últimas 48 horas, limitada a GET `2xx`, excluindo `curl`, confirmou consumidores v0 ativos: `/api/stats` 14, `/api/outreach-followups` 13, `/api/email-followups` 8 e `/api/proposal-followups` 10, com tráfego até `2026-07-22T10:21:00.696025318Z`. A paridade real continua falsa e os artefactos privados continuam a exigir resolução/aceitação dos conflitos e validação comercial do owner; as políticas oficiais de `Won`/retenção/scopes, mapping e PostgreSQL/backup off-host de produção, cutover/soak e dois releases estáveis sem consumidores v0 também permanecem em falta. Estes gates impedem corretamente o cutover, a Tarefa 19 e o sentinel; nenhuma dessas ações foi executada.

---

## Revalidação autónoma em 2026-07-22T10:56:17Z

A execução retomou o `HEAD` limpo e sincronizado `5a1ac562504eb67e3eb11ac50de85ccfbcec5d97`, na branch esperada. O plano canónico, commits, testes, migrations, documentação, PR, staging, produção, backups, rollback e processos outbound foram reinspecionados antes de qualquer mutação. Não existia trabalho staged, unstaged ou untracked e o sentinel permanecia ausente.

Num PostgreSQL 16 descartável exclusivo, migrado de raiz até `0013`, a suite Python segura completa passou com `1202 passed, 1 skipped`, uma warning de depreciação da dependência Starlette e exit `0`. O frontend Node passou `37/37`; Ruff no delta Python, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram, com zero leaks em 162 commits. Numa segunda instância exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)`, `alembic check` não detetou operações pendentes e um dump custom-format local restaurou com schema `0013`, 15 tabelas obrigatórias, zero workspaces e zero violações. O backup off-host pós-`d061ce1`, mode `0600`, 236.968 bytes e SHA-256 `4103bcdc48f65e493187f780467f3c6a016713253f0067e1b4a1fcb5834b4309`, foi novamente restore-verificado em PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Todos os recursos descartáveis desta execução foram removidos e as portas `56020`, `56021` e `56022` ficaram livres.

O staging continua na imagem funcional exata `crm-staging:d061ce1`; aplicação e PostgreSQL estão `healthy`, com zero restarts, schema `0013`, um workspace, 46 Accounts, 46 Contacts, 65 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks e 151 Activities. Existem zero violações da invariante de Account, zero Outbox rows e zero eventos `failed`/`dead_letter`. `/up` devolveu `200`; conteúdo protegido sem credenciais devolveu `401`; Leads, Contas, Propostas, Inteligência, Operações e APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`. Um soak read-only adicional executou 90 pedidos com zero falhas, p95 `49,5 ms`, máximo `68,5 ms` e zero marcadores de erro nos logs. O timer de backup permanece `enabled`/`active`, o último serviço terminou com sucesso e não existem workers, reconcilers ou publishers outbound.

Produção permaneceu inalterada e saudável no build pré-revamp: `/up=200`, `/contas=404` e `/api/stats=200`. A telemetria estruturada das últimas 48 horas, usando apenas paths exatos, respostas `2xx` e excluindo `curl`, confirmou 47 pedidos v0 ativos: `/api/stats` 16, `/api/outreach-followups` 13, `/api/email-followups` 8 e `/api/proposal-followups` 10, com tráfego até `2026-07-22T10:38:25.775166994Z`. Retirar ou proteger estes contratos no cutover atual quebraria consumidores observados.

Não existe uma lacuna técnica determinística adicional que possa ser fechada sem fabricar evidência. A comparação real continua `parity=false`; os conflitos e a amostra privada exigem resolução/aceitação do owner; a política oficial de `Won`, retenção e scopes continua por decidir; produção não tem mapping final nem PostgreSQL CRM com backup automático off-host e restore real; não houve cutover ou soak de produção. A Tarefa 19 exige depois dois releases estáveis pós-cutover, ausência comprovada de consumidores v0, export e aceitação dos stakeholders. Estes gates impedem corretamente merge, cutover, retirada do legado e criação de `.hermes/crm-revamp-complete.json`; nenhuma dessas ações foi executada.

---

## Retoma autónoma em 2026-07-22T12:01:52Z

A execução partiu do `HEAD` limpo e sincronizado `95ed618b5c429159b001c57d10d9ea845f0ef782`, na branch esperada. O plano canónico, este documento, commits, staged/unstaged work, PR, suite, migrations, staging, produção, backups, rollback e processos outbound foram reinspecionados. Não existia trabalho local por preservar e o sentinel permanecia ausente. O primeiro comando de testes usou por engano `.venv`; a inspeção do baseline confirmou o ambiente documentado `.venv311`, e a execução autoritativa foi repetida com esse interpretador.

Num PostgreSQL 16 descartável exclusivo e removido no fim, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma warning de depreciação da dependência Starlette, `PYTEST_EXIT=0` e 490,68 segundos. Os quatro módulos frontend Node passaram `37/37`. Numa segunda instância exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. O backup off-host pós-`d061ce1` foi restore-verificado numa terceira instância PostgreSQL 16: schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 163 commits e cerca de 2,14 MB sem leaks.

O staging continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL estão `healthy`, com zero restarts e schema `0013`. O smoke interno autenticado devolveu `200` e `Cache-Control: no-store` em Leads, Contas, Propostas, Inteligência, Operações, resumo do pipeline, Accounts API e Proposals API. A base mantém um workspace, 46 Accounts, 46 Contacts, 65 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks e 151 Activities; existem zero violações da invariante de Account, zero Outbox pendente e zero eventos `failed`/`dead_letter`. O timer de backup permanece `enabled`/`active`, o último serviço terminou com `success`, o rollback `crm-staging:6243331` permanece disponível e não existem workers, reconcilers ou publishers outbound. O host observado dispõe agora de cerca de 1,3 GiB de memória disponível e 11 GiB de disco livre, mas continua sem PostgreSQL ou backup CRM de produção.

Produção permaneceu inalterada e saudável no build pré-revamp: `/up=200`, `/contas=404` e `/api/stats=200`. A telemetria estruturada das últimas 48 horas, limitada aos seis paths exatos, GET `2xx` e excluindo `curl`, confirmou sete pedidos v0, incluindo tráfego em `/api/stats`, `/api/outreach-followups`, `/api/email-followups` e `/api/proposal-followups` até `2026-07-22T12:01:06Z`. O PR `#1` continua draft, mergeable e `CLEAN`, sem reviews ou checks.

Os gates de dados, decisão, produção e tempo permanecem materiais: `parity=false`; conflitos e amostra sem resolução/aceitação do owner; política oficial de `Won`, retenção e scopes por decidir; mapping final e PostgreSQL/backup automático off-host de produção ausentes; sem cutover/soak de produção; e, depois, dois releases estáveis sem consumidores v0 e aceitação dos stakeholders. A melhoria de capacidade do host não substitui esses gates. Não houve merge, deploy/migração de produção, ativação de outbound, retirada do legado ou criação de `.hermes/crm-revamp-complete.json`.

---

## Retoma autónoma em 2026-07-22T13:30:04Z

A execução partiu do `HEAD` limpo e sincronizado `5d0b6d46cdd1d7542e92a6a3daf004ce2e8311cd`, na branch esperada. O plano canónico, commits, testes, migrations, documentação, PR, staging, produção, backups, rollback, dados e processos outbound foram reinspecionados. Não existia trabalho staged, unstaged ou untracked e o sentinel permanecia ausente.

Num PostgreSQL 16 descartável e exclusivo, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma warning de depreciação da dependência Starlette, 375,41 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda instância exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)`, `alembic check` não detetou operações pendentes e um dump custom-format local restaurou com schema `0013`, 15 tabelas obrigatórias, zero workspaces e zero violações. O backup off-host pós-`d061ce1`, mode `0600`, 236.968 bytes e SHA-256 `4103bcdc48f65e493187f780467f3c6a016713253f0067e1b4a1fcb5834b4309`, foi novamente restore-verificado numa terceira instância PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff no delta Python, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 164 commits e cerca de 2,15 MB sem leaks. Todos os recursos e ficheiros temporários criados nesta execução foram removidos e as portas `56030`, `56031` e `56032` ficaram livres.

O staging continua na imagem funcional exata `crm-staging:d061ce1`; aplicação e PostgreSQL 16 estão `healthy`, com zero restarts, schema `0013`, um workspace, 46 Accounts, 46 Contacts, 65 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks e 151 Activities. Existem zero violações da invariante de Account, zero Outbox rows e todos os 216 IngestEvents estão `applied`. Contas e Propostas leem PostgreSQL, o command writer de staging usa PostgreSQL, e projeção Sheets e Agent ingress permanecem desligados. `/up` devolveu `200`; conteúdo protegido sem credenciais devolveu `401`; Leads, Contas, Propostas, Inteligência, Operações e APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`. O browser autenticado passou a 1440, 768, 390 e 320 px, sem overflow, erros de consola ou requests falhados. Um soak read-only de 90 pedidos teve zero falhas, p95 de `399,7 ms`, máximo de `516,2 ms`, zero restarts e zero marcadores de erro. O timer de backup permanece `enabled`/`active`, o último serviço terminou com `success` e não existem workers, reconcilers ou publishers outbound.

A comparação read-only atual contra a snapshot imutável permanece fail-closed: `parity=false`, 39 conflitos, 12 grupos duplicados, 66 imports esperados, 65 Leads e 46 Accounts correspondentes, um Lead e uma Account em falta, zero mismatches de fase/associação/estado/campos e 1.118 fases vazias ou não mapeadas. Os pacotes privados de revisão continuam sem referência de aceitação do owner. Nenhum conflito foi auto-fundido ou declarado resolvido.

Produção permanece no build pré-revamp `7622a2b2b8d5e0790858208b2c3a1f119edb7328`, saudável e sem PostgreSQL, mapping canónico ou timer de backup CRM de produção. `/up=200`, as novas páginas/APIs devolvem `404` e `/api/stats=200`. A telemetria estruturada das últimas 48 horas, limitada a paths exatos, GET `2xx` e excluindo `curl`, observou consumidores v0 ativos: `/api/stats` 5, `/api/outreach-followups` 6, `/api/email-followups` 4 e `/api/proposal-followups` 5, com tráfego até `2026-07-22T13:21:45Z`. O candidato não está em `main`, não existem tags/releases que o contenham e o PR `#1` continua draft, sem reviews ou checks.

Não foi encontrada uma lacuna técnica determinística adicional. Os gates explícitos que permanecem fechados não podem ser fabricados por código ou autorização autónoma: resolução ou aceitação dos conflitos, validação comercial da amostra, política oficial de `Won`, retenção e scopes, mapping final de produção, PostgreSQL CRM de produção com backup automático off-host e restore real, cutover/soak e, depois, dois releases estáveis sem consumidores v0 e aceitação dos stakeholders. Por isso não houve merge, deploy/migração de produção, ativação de outbound, retirada do legado nem criação de `.hermes/crm-revamp-complete.json`.

---

## Gate autónomo em 2026-07-22T15:30:40Z

A execução retomou a branch esperada no `HEAD` limpo e sincronizado `23416ffd28e1c40c6d4af355f263c1b9e7e0ca37`. O PR `#1` aponta para o mesmo SHA, permanece draft, mergeable e `CLEAN`, sem checks ou decisão de review. O staging funcional `d061ce1` é antecessor do `HEAD`; o único ficheiro alterado desde essa revisão é este documento. O sentinel permanecia ausente.

Os gates autoritativos do candidato passaram novamente: suite Python `1202 passed, 1 skipped`; frontend Node `37/37`; lifecycle PostgreSQL 16 `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current = 0013 (head)`; `alembic check` sem operações pendentes; restore PostgreSQL 16 com 15 tabelas obrigatórias e zero violações; Ruff, `compileall`, `node --check`, `git diff --check` e Gitleaks sem leaks. Os wrappers que devolveram estados contraditórios foram decompostos e cada componente autoritativo foi repetido isoladamente com exit `0`.

O staging continua saudável na imagem `crm-staging:d061ce1`, com PostgreSQL em `0013`, backup automático ativo, rollback `crm-staging:6243331` preservado e sem workers, reconcilers, projeção Sheets, Agent ingress ou publishers outbound. O backup off-host pós-`d061ce1`, mode `0600`, 236.968 bytes e SHA-256 `4103bcdc48f65e493187f780467f3c6a016713253f0067e1b4a1fcb5834b4309`, voltou a restaurar em PostgreSQL 16 com um workspace e zero violações. A revisão privada continua sem aceitação: `decision=0/68` e `review_notes=0/68`; os ficheiros representativos de staging contêm 46 Accounts e 44 Proposals, sem campos de decisão preenchíveis.

Produção permanece deliberadamente inalterada: `/up=200`, `/contas=404` e `/api/stats=200`; não existem container PostgreSQL, timer de backup CRM ou mapping canónico de produção. Não existem tags/releases que contenham o candidato e apenas a branch remota o contém. A janela efetivamente retida no proxy continuou a mostrar consumidores não-`curl` com respostas `2xx` nos contratos v0, portanto a retirada do legado quebraria tráfego observado.

Os gates globais continuam fechados por evidência não fabricável: `parity=false`, 39 conflitos, 12 grupos duplicados, uma Account e um Lead em falta, 1.118 fases vazias/não mapeadas, owner review sem decisões, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e infraestrutura PostgreSQL/backup de produção ausentes, sem cutover/soak e sem dois releases estáveis pós-cutover. O host também confirma zero recursos CRM de produção. Consequentemente, produção, contratos v0 e sentinel não foram alterados. O job local autónomo `318572f3de7c` permanece ativo a cada 12 horas para reavaliar os gates a partir de evidência real.

---

## Retoma autónoma em 2026-07-22T15:55:44Z

A execução partiu do `HEAD` limpo e sincronizado `ddfe01e982904369c16490298b47ccbb845ce91b`, na branch esperada. O plano canónico, commits, tests, migrations, PR, staging, produção, backup, rollback, telemetria e processos outbound foram reinspecionados. Não existia trabalho staged, unstaged ou untracked, o PR `#1` apontava para o mesmo SHA e o sentinel permanecia ausente.

Num PostgreSQL 16 descartável e exclusivo em `127.0.0.1:56040`, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma warning de depreciação da dependência Starlette, 352,84 segundos e `PYTEST_EXIT=0`. Numa segunda instância exclusiva em `56041`, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)`, `alembic check` não detetou operações pendentes e um dump custom-format restaurou com schema `0013`, 15 tabelas obrigatórias, zero workspaces e zero violações. Os quatro módulos frontend Node passaram `37/37`; Ruff no delta Python, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram. Os dois containers e o dump descartável foram removidos e as portas `56040` e `56041` ficaram livres.

O staging continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL estão healthy, o timer de backup está enabled/active e não existem workers, reconcilers ou publishers outbound. A consulta agregada direta confirmou schema `0013`, um workspace, 46 Accounts, 65 Leads, 44 Proposals, 21 Tasks, 151 Activities, zero Outbox rows e zero Leads em fases que exigem Account sem associação. `/up` devolveu `200` e Leads, Contas, Propostas, Inteligência e Operações sem credenciais devolveram `401`.
...
## Revalidação autónoma em 2026-07-22T16:59:50Z

A execução partiu do `HEAD` limpo e sincronizado `d528a9cfc65b1eda8637a98844b73c67666509e9`, na branch esperada. O plano canónico, commits, suite, migrations, PR, staging, produção, backup, owner review, telemetria e processos outbound foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; os recursos locais preexistentes e desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma warning de dependência e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda base exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. Um dump local e o backup off-host pós-`d061ce1` foram restore-verificados em PostgreSQL 16; o segundo confirmou schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram; os recursos descartáveis desta execução foram removidos. O format check global continua a apontar seis ficheiros históricos sem alteração funcional nesta retoma.

O staging permanece na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL estão `healthy`, com zero restarts, schema `0013`, um workspace, 46 Accounts, 46 Contacts, 65 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks e 151 Activities. Existem zero violações da invariante de Account, zero Outbox rows, 216 IngestEvents aplicados e zero eventos `failed`/`dead_letter`. O timer de backup está `enabled`/`active`, o último serviço terminou com sucesso e não existem workers, reconcilers ou publishers outbound. Leads, Contas, Propostas, Inteligência, Operações e três APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`; conteúdo protegido sem credenciais devolveu `401`.

Produção permaneceu inalterada no build pré-revamp, com `/up=200`, `/contas=404` e `/api/stats=200`. A telemetria estruturada das últimas 48 horas, limitada aos seis paths exatos, GET `2xx` e excluindo `curl`, confirmou 80 pedidos v0 ativos: `/api/stats` 44, `/api/portfolio` 4, `/api/recommendations` 4, `/api/outreach-followups` 11, `/api/email-followups` 7 e `/api/proposal-followups` 10, com tráfego até `2026-07-22T16:50:55Z`. Não existem tags ou releases que contenham o candidato.

A primeira tarefa formalmente incompleta continua a ser a Tarefa 19, mas o seu gate proíbe retirar contratos com consumidores observados e exige dois releases estáveis, export e aceitação. Permanecem também fechados os gates anteriores: `parity=false`, conflitos sem resolução/aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL/backup automático off-host de produção ausentes, sem cutover ou soak. Produção não foi alterada e `.hermes/crm-revamp-complete.json` não foi criado.

---

## Revalidação autónoma em 2026-07-22T17:28:41Z

A execução partiu do `HEAD` limpo e sincronizado `be715ba35251c477764174da7e9c5f1e561c6ef2`, na branch esperada. O plano canónico, `CURRENT_STATE.md`, commits, suite, migrations, PR, staging, produção, backups, rollback, owner review, telemetria e processos outbound foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; os recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 482,34 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda instância exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. Um dump local e o backup off-host pós-`d061ce1` foram restore-verificados em PostgreSQL 16; o segundo confirmou schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram, com zero leaks em 169 commits. A imagem Linux AMD64 do candidato foi reconstruída e o smoke com defaults fail-closed confirmou `/up=200`, dashboard e páginas ricas em `403`, Agent ingress em `404`, zero restarts e zero marcadores de erro. Todos os recursos descartáveis desta execução foram removidos e as portas `56050`, `56051` e `58050` ficaram livres.

O staging permanece na imagem funcional exata `crm-staging:d061ce1`; o único delta até ao `HEAD` é este documento. Aplicação e PostgreSQL estão `healthy`, com zero restarts, schema `0013`, um workspace, 46 Accounts, 46 Contacts, 65 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks e 151 Activities. Existem zero violações da invariante de Account, zero Outbox rows e zero eventos `failed`/`dead_letter`. Leads, Contas, Propostas, Inteligência, Operações e três APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`; conteúdo protegido sem credenciais devolveu `401`. O timer de backup está `enabled`/`active`, o último serviço terminou com sucesso, os logs das duas horas observadas tinham zero marcadores de erro e não existem workers, reconcilers ou publishers outbound.

Produção permaneceu inalterada no build pré-revamp, com `/up=200`, `/contas=404` e `/api/stats=200`. A telemetria estruturada das últimas 48 horas, limitada aos seis paths exatos, GET `2xx` e excluindo `curl`, confirmou consumidores v0 ativos: `/api/stats` 58, `/api/portfolio` 4, `/api/recommendations` 4, `/api/outreach-followups` 15, `/api/email-followups` 9 e `/api/proposal-followups` 13, com tráfego até `2026-07-22T17:19:26Z`. As cópias privadas de owner review continuam com zero decisões em 46 Accounts e 44 Proposals.

Não foi encontrada uma lacuna técnica determinística adicional. Continuam fechados os gates que não podem ser fabricados por código: `parity=false` e conflitos sem resolução/aceitação; owner review em `0/90`; políticas oficiais de `Won`, retenção e scopes por decidir; mapping e PostgreSQL CRM de produção com backup automático off-host e restore real ausentes; sem cutover ou soak de produção. A Tarefa 19 exige depois dois releases estáveis pós-cutover, ausência comprovada de consumidores v0, export e aceitação dos stakeholders. Produção não foi alterada e `.hermes/crm-revamp-complete.json` não foi criado.

---

## Retoma autónoma em 2026-07-22T17:56:41Z

A execução partiu do `HEAD` limpo e sincronizado `bc6c64612a432a889a7f0b8baa4be123b00488c7`, na branch esperada. O plano canónico, `CURRENT_STATE.md`, commits, suite, migrations, PR, staging, produção, backups, rollback, owner review, telemetria e processos outbound foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; o sentinel permanecia ausente e os recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 214,41 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda instância exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)`, `alembic check` não detetou operações pendentes e um dump custom-format restaurou com schema `0013`, 15 tabelas obrigatórias, zero workspaces e zero violações. O backup off-host pós-`d061ce1`, mode `0600`, 236.968 bytes e SHA-256 `4103bcdc48f65e493187f780467f3c6a016713253f0067e1b4a1fcb5834b4309`, foi novamente restore-verificado numa terceira instância PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff no delta Python, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 170 commits e cerca de 2,17 MB sem leaks. Os três containers e ficheiros descartáveis criados nesta execução foram removidos.

O staging continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL estão `healthy`, com zero restarts, schema `0013`, um workspace, 46 Accounts, 46 Contacts, 65 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks e 151 Activities. Existem zero violações da invariante de Account, zero Outbox rows e zero eventos `failed`/`dead_letter`. Leads, Contas, Propostas, Inteligência, Operações e três APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`; o soak read-only executou 90 pedidos com zero falhas, p95 `103,9 ms`, máximo `162,6 ms` e zero marcadores de erro. O timer de backup está `enabled`/`active`, o último serviço terminou com sucesso e não existem executáveis de worker, reconciler ou publisher outbound. As cópias privadas de owner review continuam com `0/46` decisões de Accounts e `0/44` decisões de Proposals.

Produção permaneceu inalterada no build pré-revamp, com `/up=200`, `/contas=404` e `/api/stats=200`; o host continua sem recurso PostgreSQL/backup CRM de produção. A telemetria estruturada das últimas 48 horas, limitada aos seis paths exatos, GET `2xx` e excluindo `curl`, confirmou consumidores v0 ativos: `/api/stats` 49, `/api/portfolio` 5, `/api/recommendations` 5, `/api/outreach-followups` 17, `/api/email-followups` 9 e `/api/proposal-followups` 14, com tráfego até `2026-07-22T17:50:54Z`.

Não existe uma ação técnica segura que feche os gates restantes sem fabricar dados, decisões ou tempo. Permanecem `parity=false`, conflitos reais sem resolução/aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL CRM de produção com backup automático off-host e restore real ausentes, sem cutover/soak e sem dois releases estáveis sem consumidores v0. Retirar o legado agora quebraria tráfego observado. Produção não foi alterada e `.hermes/crm-revamp-complete.json` não foi criado.

---

## Retoma autónoma em 2026-07-22T19:57:50Z

A execução retomou o `HEAD` limpo e sincronizado `5b9765116242778f4e021e918a56c864dced1f58`, na branch esperada. O plano canónico, commits, suite, migrations, PR, staging, produção, backup, rollback, owner review, telemetria e processos outbound foram reinspecionados. Não existia trabalho staged, unstaged ou untracked e o sentinel permanecia ausente.

Num PostgreSQL 16 descartável exclusivo, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 404,44 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda instância exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)`, `alembic check` não detetou operações pendentes e um dump local restaurou com schema `0013`, 15 tabelas obrigatórias, zero workspaces e zero violações. O backup off-host pós-`d061ce1`, mode `0600`, 236.968 bytes e SHA-256 `4103bcdc48f65e493187f780467f3c6a016713253f0067e1b4a1fcb5834b4309`, foi novamente restore-verificado numa terceira instância PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 171 commits e cerca de 2,17 MB sem leaks. Os três containers descartáveis foram removidos e as portas `56070`, `56071` e `56072` ficaram livres.

O staging continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL estão `healthy`, com zero restarts, schema `0013`, um workspace, 46 Accounts, 46 Contacts, 65 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks e 151 Activities. Existem zero violações da invariante de Account, zero Outbox rows e todos os 216 IngestEvents estão `applied`. O timer de backup está `enabled`/`active`, o último serviço terminou com sucesso e não existem workers, reconcilers ou publishers outbound. As cópias privadas de owner review continuam com `0/46` decisões de Accounts e `0/44` decisões de Proposals.

Produção permaneceu inalterada no build pré-revamp, com `/up=200`, `/contas=404` e `/api/stats=200`; o host continua sem recursos PostgreSQL/backup CRM de produção. A telemetria estruturada das últimas 48 horas, limitada aos seis paths exatos, GET `2xx` e excluindo `curl`, confirmou consumidores v0 ativos: `/api/stats` 60, `/api/portfolio` 5, `/api/recommendations` 5, `/api/outreach-followups` 19, `/api/email-followups` 11 e `/api/proposal-followups` 16, com tráfego até `2026-07-22T19:50:54Z`.

A primeira tarefa formalmente incompleta continua a ser a Tarefa 19, mas os gates anteriores de dados e produção também continuam fechados. `parity=false`, conflitos sem resolução/aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes, mapping e PostgreSQL/backup automático off-host de produção, cutover e soak permanecem ausentes. A Tarefa 19 exige ainda dois releases estáveis pós-cutover, ausência comprovada de consumidores v0, export e aceitação dos stakeholders. Não houve merge, deploy/migração de produção, ativação outbound, retirada do legado nem criação de `.hermes/crm-revamp-complete.json`.

---

## Revalidação autónoma em 2026-07-22T21:27:43Z

A execução partiu do `HEAD` limpo e sincronizado `c17705b531e38f969bbd70003c82401898f2052f`, na branch esperada. O plano canónico, commits, suite, migrations, PR, staging, produção, backup, rollback, owner review, telemetria e processos outbound foram reinspecionados. Não existia trabalho staged, unstaged ou untracked e o sentinel permanecia ausente. Os recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo, migrado de raiz até `0013`, a suite Python completa produziu `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência e zero falhas em 400,99 segundos. O wrapper terminou depois com exit `1` apenas porque tentou atribuir à variável reservada `status` do zsh; a execução pytest anterior no mesmo candidato funcional já tinha exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`, e `compileall`, `node --check` e `git diff --check` passaram com exit explícito `0`. Numa segunda instância exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. O backup off-host pós-`d061ce1`, mode `0600`, 236.968 bytes e SHA-256 `4103bcdc48f65e493187f780467f3c6a016713253f0067e1b4a1fcb5834b4309`, foi novamente restore-verificado numa terceira instância PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Todos os recursos descartáveis criados nesta execução foram removidos.

O staging continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL estão `healthy`, com zero restarts, schema `0013`, um workspace, 46 Accounts, 46 Contacts, 65 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks e 151 Activities. Existem zero violações da invariante de Account, zero Outbox rows e zero eventos `failed`/`dead_letter`. Leads, Contas, Propostas, Inteligência, Operações e três APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`; conteúdo protegido sem credenciais devolveu `401`. Um soak read-only executou 90 pedidos com zero falhas, p95 `89,1 ms`, máximo `122,2 ms` e zero marcadores de erro. O timer de backup está `enabled`/`active`, o último serviço terminou com sucesso e não existem workers, reconcilers ou publishers outbound.

Produção permaneceu inalterada no build pré-revamp, com `/up=200`, `/contas=404` e `/api/stats=200`; o host continua sem recursos PostgreSQL/backup CRM de produção. A telemetria estruturada das últimas 48 horas, limitada aos seis paths exatos, GET `2xx` e excluindo `curl`, confirmou consumidores v0 ativos: `/api/stats` 60, `/api/portfolio` 5, `/api/recommendations` 5, `/api/outreach-followups` 19, `/api/email-followups` 11 e `/api/proposal-followups` 16. As cópias privadas de owner review permanecem mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals.

Não existe uma ação técnica segura que feche os gates restantes sem fabricar dados, decisões ou tempo. Permanecem `parity=false`, conflitos reais sem resolução/aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL CRM de produção com backup automático off-host e restore real ausentes, sem cutover/soak e sem dois releases estáveis sem consumidores v0. Retirar o legado agora quebraria tráfego observado. Produção não foi alterada e `.hermes/crm-revamp-complete.json` não foi criado.

---

## Retoma autónoma em 2026-07-22T22:29:22Z

A execução partiu do `HEAD` limpo e sincronizado `d8efb55766fdf6c22aa3feb1699c9821dec9c993`, na branch esperada. O plano canónico, commits, suite, migrations, PR, staging, produção, backup, rollback, owner review, telemetria e processos outbound foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; o sentinel permanecia ausente e os recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 240,10 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda instância exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. O backup off-host pós-`d061ce1`, mode `0600`, 236.968 bytes e SHA-256 `4103bcdc48f65e493187f780467f3c6a016713253f0067e1b4a1fcb5834b4309`, foi restore-verificado numa terceira instância PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff no delta Python, `compileall`, `node --check` e `git diff --check` passaram. Todos os recursos descartáveis desta execução foram removidos.

O staging continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL estão `healthy`, o timer de backup está `enabled`/`active` e o último serviço terminou com `success`. A consulta agregada direta confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 65 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 151 Activities, zero Outbox rows, zero eventos `failed`/`dead_letter` e zero Leads em fases que exigem Account sem associação. `/up` devolveu `200`; conteúdo protegido sem credenciais devolveu `401` com `no-store`. Não existem workers, reconcilers ou publishers outbound.

Produção permaneceu inalterada no build pré-revamp `7622a2b2b8d5e0790858208b2c3a1f119edb7328`, com `/up=200`, `/contas=404` e `/api/stats=200`; o host continua sem recursos PostgreSQL/backup CRM de produção. A telemetria estruturada das últimas 48 horas, limitada aos seis paths exatos, respostas `2xx` e excluindo `curl`, confirmou consumidores v0 ativos: `/api/stats` 61, `/api/portfolio` 5, `/api/recommendations` 5, `/api/outreach-followups` 19, `/api/email-followups` 11 e `/api/proposal-followups` 16, com tráfego até `2026-07-22T22:25:13Z`. As cópias privadas de owner review permanecem mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals.

A implementação determinística até à Tarefa 18 permanece verde. A primeira tarefa formalmente incompleta continua a ser a Tarefa 19, mas os gates anteriores de dados e produção também continuam fechados: `parity=false`, conflitos sem resolução/aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes, mapping e PostgreSQL/backup automático off-host de produção, cutover e soak permanecem ausentes. A Tarefa 19 exige ainda dois releases estáveis pós-cutover, ausência comprovada de consumidores v0, export e aceitação dos stakeholders. Produção não foi alterada e `.hermes/crm-revamp-complete.json` não foi criado.

---

## Retoma autónoma em 2026-07-23T00:29:21Z

A execução partiu do `HEAD` limpo e sincronizado `2d7eba53354c587029405cce6f576c255ad9a02d`, na branch esperada. O plano canónico, commits, suite, migrations, PR, staging, produção, backup, rollback, owner review, telemetria e processos outbound foram reinspecionados. Não existia trabalho staged, unstaged ou untracked e o sentinel permanecia ausente. Os muitos containers locais preexistentes foram preservados como trabalho desconhecido.

Num PostgreSQL 16 descartável exclusivo, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 242,87 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda instância exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)`, `alembic check` não detetou operações pendentes e o backup off-host pós-`d061ce1` foi restore-verificado com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff no delta Python, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 174 commits e cerca de 2,18 MB sem leaks. Os recursos descartáveis foram removidos e as portas `56080` e `56081` ficaram livres.

O staging continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL estão `healthy`, o timer de backup está `enabled`/`active` e o último serviço terminou com `success`. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 65 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 151 Activities, zero Outbox rows, zero eventos `failed`/`dead_letter` e zero Leads em fases que exigem Account sem associação. Conteúdo protegido sem credenciais devolveu `401`; Leads, Contas, Propostas, Inteligência, Operações e APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`. Um soak read-only executou 90 pedidos com zero falhas, p95 `93,0 ms`, máximo `114,2 ms` e zero marcadores de erro. Não existem workers, reconcilers ou publishers outbound.

A comparação read-only atual contra a snapshot preservada continua fail-closed: `parity=false`, 39 conflitos, 12 grupos duplicados, 66 imports esperados, 65 Leads e 46 Accounts correspondentes, uma Account e um Lead em falta, zero mismatches de fase/associação/estado/campos e 1.118 fases vazias ou não mapeadas. As cópias privadas de owner review permanecem mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals.

Produção permanece inalterada no build pré-revamp `7622a2b2b8d5e0790858208b2c3a1f119edb7328`, com `/up=200`, `/contas=404` e `/api/stats=200`; o host não tem recurso PostgreSQL/backup CRM de produção. A telemetria estruturada das últimas 48 horas, limitada aos seis paths exatos, GET `2xx` e excluindo `curl`, confirmou consumidores v0 ativos: `/api/stats` 63, `/api/portfolio` 5, `/api/recommendations` 5, `/api/outreach-followups` 23, `/api/email-followups` 15 e `/api/proposal-followups` 20, com tráfego até `2026-07-23T00:05:46Z`. O PR `#1` continua draft, mergeable, sem reviews, checks, deployments ou environments; não existem tags/releases que contenham o candidato.

---

## Retoma autónoma em 2026-07-23T02:28:58Z

A execução partiu do `HEAD` limpo e sincronizado `6a3b1831f3e32131e120857d72f6575fb5ef30d2`, na branch esperada. Não existia trabalho staged, unstaged ou untracked, o sentinel permanecia ausente e os recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 454,49 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda instância exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. Dumps custom-format foram restore-verificados com schema `0013`, 15 tabelas obrigatórias e zero violações. Ruff no delta Python, `compileall`, `node --check` e `git diff --check` passaram. Todos os recursos descartáveis criados nesta execução foram removidos.

O staging continua na imagem funcional `crm-staging:d061ce1`; o delta funcional até ao `HEAD` é nulo. Aplicação e PostgreSQL 16 estão `healthy`, com zero restarts, schema `0013`, um workspace, 46 Accounts, 46 Contacts, 65 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 151 Activities, zero Outbox rows, 216 IngestEvents `applied` e zero violações da invariante de Account. Leads, Contas, Propostas, Inteligência, Operações e APIs v1 devolveram `200` autenticado; conteúdo protegido sem credenciais devolveu `401` com `no-store`. O timer de backup está `enabled`/`active`, o último serviço terminou com `success` e não existem workers, reconcilers ou publishers outbound.

Foi criado o backup de staging `crm-staging-20260723T022504Z.dump`, copiado off-host para `/Users/max/.hermes/profiles/marketing-max/backups/crm/staging/` com mode `0600`, 236.968 bytes e SHA-256 `eeed726d46be47f77117add8daea6a825df7d9ccec67f0f4f4c81c3ed1f5c2ba`. Os hashes remoto e local coincidiram e o arquivo exato foi restore-verificado num PostgreSQL 16 separado com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações.

As cópias privadas de owner review continuam mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals. Produção permanece no build pré-revamp, sem PostgreSQL/backup CRM nem mapping canónico. A telemetria das últimas 48 horas registou novo consumo v0 em `/api/stats` às `2026-07-23T02:21:40.689485403Z`. Permanecem `parity=false`, 39 conflitos, uma Account e um Lead em falta, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, sem cutover/soak e sem dois releases estáveis pós-cutover. A Tarefa 19 e o sentinel continuam corretamente bloqueados; produção não foi alterada.

Não existe uma ação técnica segura que feche os gates restantes sem fabricar dados, decisões ou tempo. Permanecem `parity=false`, conflitos sem resolução/aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL CRM de produção com backup automático off-host e restore real ausentes, sem cutover/soak e sem dois releases estáveis sem consumidores v0. Retirar o legado agora quebraria tráfego observado. Produção não foi alterada e `.hermes/crm-revamp-complete.json` não foi criado.

---

## Retoma autónoma em 2026-07-23T03:27:23Z

A execução partiu do `HEAD` limpo e sincronizado `15b31735bd0ad964e7591b92c9d5edefa94a2aeb`, na branch esperada. O plano canónico, commits, suite, migrations, PR, staging, produção, backup, rollback, owner review, telemetria e processos outbound foram reinspecionados. Não existia trabalho staged, unstaged ou untracked e o sentinel permanecia ausente. Os recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 236,79 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda instância exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. O backup off-host de staging `crm-staging-20260723T022504Z.dump`, mode `0600`, 236.968 bytes e SHA-256 `eeed726d46be47f77117add8daea6a825df7d9ccec67f0f4f4c81c3ed1f5c2ba`, foi restore-verificado em PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff no delta Python, `compileall`, `node --check`, `git diff --check`, Gitleaks e o scan de linhas adicionadas em caminhos de produção passaram; Gitleaks analisou 176 commits e cerca de 2,19 MB sem leaks. Todos os recursos descartáveis criados nesta execução foram removidos.

O staging no proxy/TLS final continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL 16 estão `healthy`, com zero restarts. As flags mantêm reads e command writer em PostgreSQL, projeção Sheets e Agent ingress desligados. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 65 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 151 Activities, zero Outbox rows, zero eventos `failed`/`dead_letter` e zero Leads em fases que exigem Account sem associação. `/up` devolveu `200`, conteúdo protegido sem credenciais devolveu `401`, o timer diário de backup está `enabled`/`active` com último resultado `success`, os logs das duas horas observadas tinham zero marcadores de erro e não existem workers, reconcilers ou publishers outbound.

Produção permaneceu inalterada no build pré-revamp, com `/up=200`, `/contas=404` e `/api/stats=200`; o host continua sem recursos PostgreSQL/backup CRM de produção. A telemetria estruturada das últimas 48 horas, limitada aos seis paths exatos, GET `2xx` e excluindo `curl`, observou novo consumo v0 em `/api/stats` às `2026-07-23T02:21:40.689485403Z`. As cópias privadas de owner review permanecem mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals. O PR `#1` continua draft, mergeable, sem reviews, checks, deployments ou environments.

Não foi encontrada uma lacuna técnica determinística adicional. Continuam fechados os gates que não podem ser fabricados por código ou pela autorização autónoma: `parity=false`, 39 conflitos e uma Account/Lead em falta sem resolução ou aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL CRM de produção com backup automático off-host e restore real ausentes, sem cutover/soak e sem dois releases estáveis pós-cutover sem consumidores v0. Retirar o legado agora quebraria tráfego observado. Produção não foi alterada e `.hermes/crm-revamp-complete.json` não foi criado.

---

## Retoma autónoma em 2026-07-23T03:59:39Z

A execução partiu do `HEAD` limpo e sincronizado `2a7c644a1aa34303c06adfd2371eef0f86c2dbf5`, na branch esperada. O plano canónico, commits, suite, migrations, PR, staging, produção, backup, rollback, owner review, telemetria e processos outbound foram reinspecionados. Não existia trabalho staged, unstaged ou untracked, o sentinel permanecia ausente e todos os recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 248,36 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda instância exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. Um dump da base exercitada foi restore-verificado com schema `0013`, 15 tabelas obrigatórias, 23 workspaces de fixtures e zero violações. Ruff no delta Python, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 177 commits e cerca de 2,19 MB sem leaks. Os recursos descartáveis criados nesta execução foram removidos.

O staging continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL 16 estão `healthy`, com zero restarts. A consulta autenticada confirmou `200` e `Cache-Control: no-store` em Leads, Contas, Propostas, Inteligência, Operações e três APIs v1; os mesmos conteúdos sem credenciais devolveram `401`. A base mantém schema `0013`, um workspace, 46 Accounts, 46 Contacts, 65 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 151 Activities, zero Outbox rows, zero eventos `failed`/`dead_letter` e zero violações da invariante de Account. O timer de backup está `enabled`/`active`, o último serviço terminou com `success`, os logs das duas horas observadas tinham zero marcadores de erro e não existem workers, reconcilers ou publishers outbound. O backup off-host `crm-staging-20260723T022504Z.dump`, mode `0600` e SHA-256 `eeed726d46be47f77117add8daea6a825df7d9ccec67f0f4f4c81c3ed1f5c2ba`, foi novamente restore-verificado em PostgreSQL 16 com um workspace e zero violações.

Produção permanece inalterada e saudável no build pré-revamp, com zero restarts, `/up=200`, `/contas=404` e `/api/stats=200`; não tem variáveis CRM, PostgreSQL CRM ou timer de backup CRM. A telemetria estruturada das últimas 48 horas, limitada aos seis paths exatos, GET `2xx`, host de produção e excluindo `curl`, voltou a observar consumo v0 em `/api/stats` às `2026-07-23T02:21:40.689485403Z`. O PR `#1` aponta para o SHA exato da branch, permanece draft, `MERGEABLE/CLEAN`, sem reviews, checks, releases ou tags que contenham o candidato. As cópias privadas de owner review continuam mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals.

A implementação determinística até à Tarefa 18 continua verde, mas a conclusão global não é verdadeira. Permanecem `parity=false`, conflitos reais sem resolução ou aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL CRM de produção com backup automático off-host e restore real ausentes, sem cutover/soak e sem dois releases estáveis pós-cutover ou ausência de consumidores v0. Por isso produção, contratos legados e `.hermes/crm-revamp-complete.json` não foram alterados.

---

## Retoma autónoma em 2026-07-23T05:26:11Z

A execução partiu do `HEAD` limpo e sincronizado `b0cb9721dd1d274da5ce2bec747bb20c0e821776`, na branch esperada. O plano canónico, commits, suite, migrations, PR, staging, produção, backup, rollback, owner review, telemetria e processos outbound foram reinspecionados. Não existia trabalho staged, unstaged ou untracked e o sentinel permanecia ausente. Os recursos locais preexistentes e desconhecidos foram preservados; não existiam processos locais ou remotos de worker CRM, reconciler ou outbox publisher.

Num PostgreSQL 16 descartável exclusivo, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 233,37 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda base vazia e exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. Ruff no delta Python, `compileall`, `node --check` e `git diff --check` passaram. O backup off-host de staging `crm-staging-20260723T022504Z.dump`, mode `0600` e SHA-256 `eeed726d46be47f77117add8daea6a825df7d9ccec67f0f4f4c81c3ed1f5c2ba`, foi restore-verificado num terceiro PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Os recursos descartáveis desta execução foram removidos e as portas `56100` e `56101` ficaram livres.

O staging continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL 16 estão `healthy`, com zero restarts. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 65 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 151 Activities, zero Outbox rows, zero eventos `failed`/`dead_letter` e zero violações da invariante de Account. Leads, Contas, Propostas, Inteligência, Operações e três APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`; conteúdo protegido sem credenciais devolveu `401`. O timer de backup está `enabled`/`active`, o último serviço terminou com `success` e não existem workers, reconcilers ou publishers outbound.

Produção permanece inalterada e saudável no build pré-revamp, com `/up=200`, `/contas=404` e `/api/stats=200`; o container não tem configuração CRM e o host não tem PostgreSQL/backup CRM de produção. A telemetria estruturada das últimas 48 horas, limitada aos seis paths exatos, GET `2xx`, host de produção e excluindo `curl`, observou um consumidor v0 em `/api/stats` às `2026-07-23T02:21:40.689485403Z`. As cópias privadas de owner review permanecem mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals. O PR `#1` continua draft, `MERGEABLE`, sem reviews ou checks.

A primeira tarefa formalmente incompleta continua a ser a Tarefa 19, mas os gates anteriores também permanecem fechados: `parity=false`, conflitos sem resolução/aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL CRM de produção com backup automático off-host e restore real ausentes, sem cutover/soak e sem dois releases estáveis sem consumidores v0. Retirar o legado agora quebraria tráfego observado. Produção e `.hermes/crm-revamp-complete.json` não foram alterados.

---

## Retoma autónoma em 2026-07-23T06:51:23Z

A execução retomou o `HEAD` limpo e sincronizado `8fd8a45e8a3f3a405d00d871aeb5815052d33c21`, na branch esperada. O plano canónico, commits, testes, migrations, PR, staging, produção, backups, rollback, owner review, telemetria e processos outbound foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; os recursos preexistentes desconhecidos foram preservados e o sentinel permanecia ausente.

Num PostgreSQL 16 descartável exclusivo, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 451,92 segundos e exit explícito `0`; os quatro módulos frontend Node passaram `37/37`. Numa segunda instância vazia e exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. A primeira invocação do restore verifier usou por erro uma base administrativa ainda inexistente e falhou fechada; a execução autoritativa foi repetida numa terceira instância com a base de teste criada previamente e verificou um dump custom-format com schema `0013`, 15 tabelas obrigatórias, zero workspaces e zero violações. Ruff no delta Python, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 179 commits e cerca de 2,20 MB sem leaks. Todos os recursos descartáveis desta retoma foram removidos e as portas `56110`, `56111` e `56112` ficaram livres.

O staging no proxy/TLS final continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL 16 estão `healthy`, com zero restarts. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 65 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 151 Activities, zero Outbox rows, 216 IngestEvents `applied` e zero violações da invariante de Account. `/up` devolveu `200`; Leads, Contas, Propostas, Inteligência e Operações sem credenciais devolveram `401`. O timer de backup está `enabled`/`active`, o último serviço terminou com `success` e não existem workers, reconcilers ou publishers outbound.

Produção permanece inalterada e saudável no build pré-revamp, com `/up=200`, `/contas=404` e `/api/stats=200`; o container não tem configuração CRM e o host não tem PostgreSQL/backup CRM de produção. A telemetria estruturada das últimas 48 horas, limitada aos seis paths v0 exatos, GET `2xx`, host de produção e excluindo `curl`, observou um pedido em `/api/stats` às `2026-07-23T02:21:40.689485403Z`. As cópias privadas de owner review continuam mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals. O PR `#1` permanece draft, `MERGEABLE/CLEAN`, sem reviews, checks, releases ou tags que contenham o candidato.

Não foi encontrada uma lacuna técnica determinística adicional. Permanecem fechados os gates que não podem ser fabricados por código ou autorização autónoma: `parity=false`, conflitos sem resolução/aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL CRM de produção com backup automático off-host e restore real ausentes, sem cutover/soak e sem dois releases estáveis pós-cutover ou ausência comprovada de consumidores v0. Retirar o legado agora quebraria tráfego observado. Produção não foi alterada e `.hermes/crm-revamp-complete.json` não foi criado.

---

## Retoma autónoma em 2026-07-23T07:26:24Z

A execução partiu do `HEAD` limpo e sincronizado `ef272f4d63ea343eb1dc0b61fe8ed714368b9812`, na branch esperada. O plano canónico, commits, suite, migrations, PR, staging, produção, backup, rollback, owner review, telemetria e processos outbound foram reinspecionados. Não existia trabalho staged, unstaged ou untracked e o sentinel permanecia ausente. Os recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 230,86 segundos e exit explícito `0`; os quatro módulos frontend Node passaram `37/37`. Numa segunda instância vazia e exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)`, `alembic check` não detetou operações pendentes e o backup off-host de staging `crm-staging-20260723T022504Z.dump`, mode `0600`, 236.968 bytes e SHA-256 `eeed726d46be47f77117add8daea6a825df7d9ccec67f0f4f4c81c3ed1f5c2ba`, foi restore-verificado com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff no delta Python, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 180 commits e cerca de 2,20 MB sem leaks. Os recursos descartáveis foram removidos e as portas `56120` e `56121` ficaram livres.

O staging no proxy/TLS final continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL 16 estão `healthy`, com zero restarts. As flags mantêm reads de Contas/Propostas e command writer em PostgreSQL, com projeção Sheets e Agent ingress desligados. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 65 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 151 Activities, zero Outbox rows, 216 IngestEvents `applied`, zero zeros sintéticos em valores `missing` e zero violações da invariante de Account. O timer de backup está `enabled`/`active`, o último serviço terminou com `success` e não existem workers, reconcilers ou publishers outbound. `/up` devolveu `200`; conteúdo protegido de staging sem credenciais devolveu `401`.

Produção permaneceu inalterada e saudável no build pré-revamp, com `/up=200`, `/contas=404` e `/api/stats=200`; o container não tem configuração CRM e o host não tem PostgreSQL/backup CRM de produção. A telemetria estruturada das últimas 48 horas, limitada aos seis paths v0 exatos, GET `2xx`, host de produção e excluindo `curl`, observou 10 pedidos ativos: `/api/stats` 3, `/api/outreach-followups` 3, `/api/email-followups` 2 e `/api/proposal-followups` 2, com tráfego até `2026-07-23T07:15:09.280090259Z`. As cópias privadas de owner review continuam mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals. O branch remoto recebeu o commit de evidência `316219a909ba5225d7cd29f8552747971666e416`; durante a verificação imediatamente posterior, a API do PR `#1` ainda apresentava o `head` anterior `ef272f4d63ea343eb1dc0b61fe8ed714368b9812`, permanecendo draft, `MERGEABLE/CLEAN`, sem reviews, checks, releases ou tags que contenham o candidato.

A implementação determinística até à Tarefa 18 permanece verde, mas a conclusão global continua falsa. A comparação real permanece `parity=false`, os conflitos e a amostra não têm resolução/aceitação do owner, as políticas oficiais de `Won`/retenção/scopes estão por decidir, produção não tem mapping canónico nem PostgreSQL CRM com backup automático off-host e restore real, e não houve cutover ou soak de produção. A Tarefa 19 exige depois dois releases estáveis pós-cutover, ausência comprovada de consumidores v0, export e aceitação dos stakeholders. Retirar o legado agora quebraria tráfego observado; produção e `.hermes/crm-revamp-complete.json` não foram alterados.

---

## Retoma autónoma e atualização shadow de staging em 2026-07-23T08:05:28Z

A execução partiu do `HEAD` limpo e sincronizado `bcc2565ff656d16f41d279f2da83fd98c6d86848`, na branch esperada. O plano canónico, commits, suites, migrations, PR, staging, produção, backups, rollback, owner review, telemetria e processos outbound foram reinspecionados. Não existia trabalho staged, unstaged ou untracked. Os recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 479,28 segundos e exit explícito `0`; os quatro módulos frontend Node passaram `37/37`. Numa segunda instância vazia e exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. Ruff no delta Python, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 182 commits e cerca de 2,20 MB sem leaks. Todos os recursos descartáveis criados para estes gates foram removidos.

Uma captura nova da Sheet real foi feita com o scope read-only, ficheiros temporários mode `0600` e zero writes na origem. O input atual manteve 1.247 linhas, 1.202 aplicáveis, 12 identidades duplicadas e 21 linhas sem identidade segura, mas passou a conter 67 imports de Account/Lead e 48 linhas de Proposal candidatas. Antes de aplicar qualquer alteração ao staging, o backup `crm-staging-20260723T080048Z.dump` foi criado e restore-verificado em PostgreSQL 16: 236.968 bytes, SHA-256 `9df9a404fe2711b7fff7094ed8152b242add7c842435dcc4748cb78d6739bb91`, schema `0013`, um workspace e zero violações.

Com workers, reconciler, outbox publisher, projeção Sheets e Agent ingress desligados, o backfill de contas foi aplicado duas vezes ao staging. A primeira execução persistiu sete imports novos, ligou seis a Accounts existentes, manteve 52 conflitos fail-closed e não fez merge por nome; a segunda persistiu zero imports e devolveu 66 replay no-op. O backfill de propostas encontrou 44 replay no-op e quatro Accounts sem correspondência nas duas execuções, portanto criou zero Proposals novas. A comparação read-only posterior devolveu `parity=false`, 67 imports esperados, 66 Leads correspondentes, 46 Accounts correspondentes, um Lead e uma Account em falta, 39 conflitos, 12 grupos duplicados, 1.117 fases vazias/não mapeadas e zero mismatches de fase, associação, estado ou campos de origem.

O staging ficou em `0013` com um workspace, 46 Accounts, 46 Contacts, 66 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks e 158 Activities. Permanecem zero Outbox rows, zero eventos `failed`/`dead_letter`, zero Leads em fases que exigem Account sem associação e zero zeros sintéticos em valores `missing`. O backup pós-apply `crm-staging-20260723T080238Z.dump`, 239.676 bytes e SHA-256 `fa3f089f25821719bdfe1e028780754e290bd8b075aa0a783161bbb769a4bb10`, foi copiado para verificação e restaurou em PostgreSQL 16 com schema `0013`, um workspace e zero violações. A aplicação e a base continuaram `healthy`, com zero restarts e zero marcadores de erro; o soak autenticado pós-apply executou 90 pedidos com zero falhas, p95 de `141,5 ms` e máximo de `221,9 ms`. Os snapshots e credenciais temporários foram removidos.

As cópias privadas de owner review continuam mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals. Produção permanece no build pré-revamp, sem configuração ou PostgreSQL CRM, com `/up=200`, `/contas=404` e `/api/stats=200`. A telemetria estruturada das últimas 48 horas, limitada aos seis paths v0 exatos, GET `2xx`, host de produção e excluindo `curl`, continuou a observar consumidores: `/api/stats` 3, `/api/outreach-followups` 3, `/api/email-followups` 2 e `/api/proposal-followups` 2, com tráfego até `2026-07-23T07:15:09Z`.

A atualização shadow fechou apenas os imports novos inequivocamente associáveis. Permanecem gates que não podem ser fabricados: `parity=false`, conflitos e uma Account/Lead em falta sem resolução ou aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes, mapping e PostgreSQL/backup automático off-host de produção, cutover/soak e dois releases estáveis pós-cutover sem consumidores v0. A Tarefa 19 continua bloqueada pelo seu próprio gate e `.hermes/crm-revamp-complete.json` não foi criado.

---

## Retoma autónoma em 2026-07-23T08:32:06Z

A execução retomou o `HEAD` limpo e sincronizado `86830778b129306d16223a26ec0c68f68479d7fe`, na branch esperada. O plano canónico, `CURRENT_STATE.md`, commits, suite, migrations, PR, staging, produção, backup, rollback, owner review, telemetria e processos outbound foram reinspecionados. Não existia trabalho staged, unstaged ou untracked e o sentinel permanecia ausente. Todos os recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 307,45 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37` com exit explícito `0`. Numa segunda instância vazia e exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)`, `alembic check` não detetou operações pendentes e um dump custom-format foi restore-verificado numa terceira base PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, zero workspaces e zero violações. Ruff nos 144 ficheiros Python do delta, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 183 commits e cerca de 2,21 MB sem leaks. A dívida Ruff preexistente fora do delta continua separada e não foi alterada. Os recursos descartáveis desta execução foram removidos e as portas `56130` a `56133` ficaram livres.

O staging no proxy/TLS final continua na imagem funcional `crm-staging:d061ce1`; o único delta até ao `HEAD` é este documento. A aplicação e o PostgreSQL 16 estão `healthy`, com zero restarts. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 66 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 158 Activities, zero Outbox rows, zero eventos `failed`/`dead_letter`, zero zeros sintéticos não confirmados e zero violações da invariante de Account. `/up` devolveu `200`; Leads, Contas, Propostas, Inteligência, Operações e as APIs v1 autenticadas devolveram `200` com `Cache-Control: no-store`; conteúdo protegido sem credenciais devolveu `401`. O timer de backup permanece `enabled`/`active`, o último serviço terminou com `success` e não existem workers, reconcilers ou publishers outbound.

Produção permanece inalterada e saudável no build pré-revamp `7622a2b2b8d5e0790858208b2c3a1f119edb7328`, com `/up=200`, `/contas=404` e `/api/stats=200`; o host não tem recurso PostgreSQL/backup CRM de produção. A telemetria estruturada das últimas 48 horas, limitada aos seis paths v0 exatos, GET `2xx`, host de produção e excluindo `curl`, confirmou consumidores ativos: `/api/stats` 4, `/api/outreach-followups` 3, `/api/email-followups` 2 e `/api/proposal-followups` 2, com tráfego até `2026-07-23T07:53:42.775805912Z`. As cópias privadas de owner review continuam mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals. O PR `#1` aponta para o SHA exato da branch, permanece draft e mergeable, sem reviews ou checks; não existem releases ou tags que contenham o candidato.

A implementação determinística até à Tarefa 18 continua verde, mas a conclusão global não é verdadeira. Permanecem `parity=false`, conflitos reais e uma Account/Lead em falta sem resolução ou aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL CRM de produção com backup automático off-host e restore real ausentes, sem cutover/soak de produção e sem dois releases estáveis pós-cutover ou ausência comprovada de consumidores v0. A Tarefa 19 proíbe retirar contratos nestas condições. Produção, contratos legados e `.hermes/crm-revamp-complete.json` não foram alterados.

---

## Retoma autónoma em 2026-07-23T09:03:54Z

A execução partiu do `HEAD` limpo e sincronizado `a6323b14f2346996fbff28ad42c13fdd11cf5d14`, na branch esperada. O plano canónico, `CURRENT_STATE.md`, commits, suite, migrations, PR, staging, produção, backup, owner review, telemetria, processos outbound e sentinel foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 231,98 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda base vazia e exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. Ruff nos ficheiros Python do delta, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 184 commits e cerca de 2,21 MB sem leaks. A imagem Linux AMD64 do candidato foi construída e o smoke com defaults fail-closed confirmou `/up=200`, dashboard e páginas ricas em `403`, Agent ingress em `404`, zero restarts e zero marcadores de erro. Os containers, portas e imagem descartáveis desta retoma foram removidos.

O staging permanece na imagem funcional `crm-staging:d061ce1`; o único delta funcional até ao candidato é nulo, porque os commits posteriores alteram apenas este documento. Aplicação e PostgreSQL 16 estão `healthy`, com zero restarts; o timer de backup está `enabled`/`active`, o último serviço terminou com `success` e não existem workers, reconcilers ou publishers outbound. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 66 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 158 Activities, zero Outbox rows, 223 IngestEvents `applied` e zero violações da invariante de Account. Leads, Contas, Propostas, Inteligência, Operações e três APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`.

Foi criado um backup custom-format do staging já atualizado, copiado off-host para `/Users/max/.hermes/profiles/marketing-max/backups/crm/staging/crm-staging-20260723T090031Z-post-shadow.dump`, com mode `0600`, 239.676 bytes e SHA-256 `a26eb502ca5a77422672d99ecd20a9d471fe074fbb3d78c2918a51110fac56ec`. Os hashes remoto/local coincidiram e o arquivo exato foi restore-verificado num terceiro PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações.

Produção permanece no build pré-revamp, sem configuração, PostgreSQL ou backup CRM, com `/up=200`, `/contas=404` e os contratos v0 disponíveis. A telemetria estruturada das últimas 48 horas, limitada aos seis paths exatos, GET `2xx`, host de produção e excluindo `curl` e o probe desta retoma, confirmou consumidores ativos: `/api/stats` 5, `/api/outreach-followups` 5, `/api/email-followups` 2 e `/api/proposal-followups` 2, com tráfego até `2026-07-23T08:58:52.698160565Z`. As cópias privadas de owner review continuam mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals. O PR `#1` aponta para o SHA anterior exato, permanece draft e sem checks ou reviews; não existem tags ou releases que contenham o candidato.

Os gates restantes continuam materiais e não podem ser fabricados por código ou autorização autónoma: `parity=false`, conflitos reais e uma Account/Lead em falta sem resolução ou aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL CRM de produção com backup automático off-host e restore real ausentes, sem cutover/soak de produção e sem dois releases estáveis pós-cutover ou ausência comprovada de consumidores v0. A Tarefa 19 continua bloqueada pelo próprio contrato do plano. Produção e `.hermes/crm-revamp-complete.json` não foram alterados.

---

## Retoma autónoma em 2026-07-23T10:01:32Z

A execução partiu do `HEAD` limpo e sincronizado `1e512a5bc4a4220749c42deb48263663dfcf2bdb`, na branch esperada. O plano canónico, este documento, commits, suite, migrations, PR, staging, produção, backup, owner review, telemetria, processos outbound e sentinel foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 221,92 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`; oito ficheiros JavaScript passaram `node --check`. Numa segunda base vazia exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. O backup off-host pós-shadow `crm-staging-20260723T090031Z-post-shadow.dump`, mode `0600`, 239.676 bytes e SHA-256 `a26eb502ca5a77422672d99ecd20a9d471fe074fbb3d78c2918a51110fac56ec`, foi restore-verificado numa terceira instância PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Os três containers descartáveis foram removidos e as portas `56140` a `56142` ficaram livres.

O staging no proxy/TLS final continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL 16 estão `healthy`, com zero restarts. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 66 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 158 Activities, zero Outbox rows, 223 IngestEvents `applied` e zero violações da invariante de Account. Leads, Contas, Propostas, Inteligência, Operações e três APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`; sem credenciais o conteúdo protegido devolveu `401`. O timer de backup está `enabled`/`active`, o último serviço terminou com `success` e não existem workers, reconcilers ou publishers outbound.

Produção permanece inalterada no build pré-revamp, sem configuração, PostgreSQL ou backup CRM, com `/up=200`, `/contas=404` e `/api/stats=200`. A telemetria estruturada das últimas 48 horas, limitada aos seis paths v0 exatos, GET `2xx`, host de produção e excluindo `curl`, confirmou consumidores ativos: `/api/stats` 22, `/api/portfolio` 1, `/api/recommendations` 1, `/api/outreach-followups` 10, `/api/email-followups` 5 e `/api/proposal-followups` 6, com tráfego até `2026-07-23T09:58:52Z`. As cópias privadas de owner review continuam mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals. O PR `#1` aponta para o SHA exato anterior, permanece draft e mergeable, sem reviews ou checks.

A implementação determinística até à Tarefa 18 permanece verde. A primeira tarefa formalmente incompleta continua a ser a Tarefa 19, mas os gates anteriores também permanecem fechados: `parity=false`, conflitos sem resolução/aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL/backup automático off-host de produção ausentes, sem cutover/soak e sem dois releases estáveis sem consumidores v0. Retirar o legado agora quebraria tráfego observado. Produção e `.hermes/crm-revamp-complete.json` não foram alterados.

---

## Retoma autónoma em 2026-07-23T11:04:06Z

A execução partiu do `HEAD` limpo e sincronizado `df715e7d1d810e8a976f4eed3293c1ec6ff6ef76`, na branch esperada. O plano canónico, `CURRENT_STATE.md`, commits, suites, migrations, PR, staging, produção, backup, owner review, processos outbound e sentinel foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 184,72 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Um downgrade tentado sobre a base já populada pela suite falhou fechada como desenhado porque existiam transições de fase estruturadas; o lifecycle autoritativo foi repetido numa segunda base vazia exclusiva e passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`. `alembic current` devolveu `0013 (head)`, `alembic check` não detetou operações pendentes e um dump custom-format local foi restore-verificado com schema `0013`, 15 tabelas obrigatórias, zero workspaces e zero violações. Ruff no delta Python, `compileall`, `node --check` e `git diff --check` passaram. Os dois containers e o dump descartáveis foram removidos; as portas `56150` e `56151` ficaram livres.

O staging no proxy/TLS final continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL 16 estão `healthy`, com zero restarts e zero marcadores de erro nas duas horas observadas. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 66 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 158 Activities, zero Outbox rows, zero eventos `failed`/`dead_letter` e zero violações da invariante de Account. `/up` devolveu `200`; conteúdo protegido sem credenciais devolveu `401`; Leads, Contas, Propostas, Inteligência, Operações e três APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`. Projeção Sheets e Agent ingress permanecem desligados e não existem workers, reconcilers ou publishers outbound. O timer de backup está `enabled`/`active`, com último resultado `success`. O backup off-host pós-shadow `crm-staging-20260723T090031Z-post-shadow.dump`, mode `0600`, 239.676 bytes e SHA-256 `a26eb502ca5a77422672d99ecd20a9d471fe074fbb3d78c2918a51110fac56ec`, foi novamente restore-verificado em PostgreSQL 16 com um workspace e zero violações.

Produção permanece inalterada no build pré-revamp: `/up=200`, dashboard legado em `/=200`, novas páginas/APIs em `404` e `/api/stats=200`; não existe PostgreSQL/backup CRM de produção. O PR `#1` aponta para o SHA exato da branch, permanece draft e mergeable, sem reviews, checks, deployments, tags ou releases que contenham o candidato. As cópias privadas de owner review continuam mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals. A evidência estruturada imediatamente anterior nesta mesma janela continua a mostrar consumidores v0 até `2026-07-23T09:58:52Z`.

A implementação determinística até à Tarefa 18 continua verde, mas a conclusão global permanece falsa. Persistem `parity=false`, 39 conflitos e uma Account/Lead em falta sem resolução ou aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL CRM de produção com backup automático off-host e restore real ausentes, sem cutover/soak de produção e sem dois releases estáveis pós-cutover ou ausência comprovada de consumidores v0. Estes gates técnicos, de dados, humanos e temporais impedem corretamente merge, produção, retirada do legado e criação de `.hermes/crm-revamp-complete.json`.

---

## Retoma autónoma em 2026-07-23T12:29:02Z

A execução partiu do `HEAD` limpo e sincronizado `9c1d4b7678c2319ac85eafd41fae074024ca9126`, na branch esperada. O plano canónico, este documento, commits, suites, migrations, PR, staging, produção, backup, owner review, telemetria, processos outbound e sentinel foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo em loopback, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 245,94 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37` com exit explícito `0`. Numa segunda instância vazia exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. O backup off-host pós-shadow `crm-staging-20260723T090031Z-post-shadow.dump` foi restore-verificado em PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff no delta Python, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 187 commits e cerca de 2,22 MB sem leaks. Os recursos descartáveis desta execução foram removidos e a porta `56160` ficou livre.

O staging continua na imagem funcional `crm-staging:d061ce1`; o único delta posterior é documentação. Aplicação e PostgreSQL 16 estão `healthy`, com zero restarts; o timer diário de backup está `enabled`/`active`, com último resultado `success`, e não existem workers, reconcilers ou publishers outbound. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 66 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 158 Activities, zero Outbox rows, zero eventos `failed`/`dead_letter` e zero violações da invariante de Account. `/up`, Leads, Contas, Propostas, Inteligência, Operações e três APIs v1 devolveram `200` autenticado; as superfícies protegidas devolveram `Cache-Control: no-store` e os pedidos sem credenciais testados devolveram `401`.

Produção permanece inalterada no build pré-revamp, sem PostgreSQL/backup CRM ou mapping canónico. `/up` devolve `200` e `/contas` devolve `404`. A telemetria estruturada das últimas 48 horas, limitada aos seis paths v0 exatos, GET `2xx`, host de produção e excluindo `curl`, confirmou consumidores ativos: `/api/stats` 28, `/api/portfolio` 1, `/api/recommendations` 1, `/api/outreach-followups` 12, `/api/email-followups` 7 e `/api/proposal-followups` 8, com tráfego até `2026-07-23T12:01:18Z`. As cópias privadas de owner review permanecem mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals. O PR `#1` permanece draft, mergeable, sem reviews, checks ou environments GitHub.

A implementação determinística até à Tarefa 18 permanece verde. A primeira tarefa formalmente incompleta continua a ser a Tarefa 19, mas os gates anteriores também permanecem fechados: `parity=false`, conflitos sem resolução/aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL/backup automático off-host de produção ausentes, sem cutover/soak e sem dois releases estáveis sem consumidores v0. Retirar o legado agora quebraria tráfego observado. Produção e `.hermes/crm-revamp-complete.json` não foram alterados.

---

## Retoma autónoma em 2026-07-23T13:32:21Z

A execução partiu do `HEAD` limpo e sincronizado `fa915a33e5cf3c864b8a4f105c26cc259a6e4142`, na branch esperada. O plano canónico, este documento, commits, suite, migrations, PR, staging, produção, backup, rollback, owner review, telemetria, processos outbound e sentinel foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo em loopback, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 447,65 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda instância vazia exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. O backup off-host pós-shadow `crm-staging-20260723T090031Z-post-shadow.dump`, mode `0600`, 239.676 bytes e SHA-256 `a26eb502ca5a77422672d99ecd20a9d471fe074fbb3d78c2918a51110fac56ec`, foi restore-verificado numa terceira instância PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff nos 145 ficheiros Python do delta, `compileall`, `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 255 commits e cerca de 4,12 MB sem leaks. Os três recursos descartáveis foram removidos e as portas `56170` a `56172` ficaram livres.

O staging continua na imagem funcional `crm-staging:d061ce1`; o único delta posterior é este documento. Aplicação e PostgreSQL 16 estão `healthy`, com zero restarts. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 66 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 158 Activities, zero Outbox rows, 223 IngestEvents `applied` e zero violações da invariante de Account. `/up`, Leads, Contas, Propostas, Inteligência, Operações e três APIs v1 devolveram `200` autenticado; as superfícies protegidas devolveram `Cache-Control: no-store`, os pedidos sem credenciais devolveram `401` e o Agent ingress desligado devolveu `404`. Um soak read-only executou `90/90` pedidos com sucesso, p95 `72,6 ms` e máximo `331,8 ms`. O timer diário de backup está `enabled`/`active`, com último resultado `success`; o rollback imediato `crm-staging:6243331` permanece disponível e não existem workers, reconcilers ou publishers outbound.

Produção permanece inalterada no build pré-revamp, sem PostgreSQL/backup CRM ou mapping canónico. `/up` devolve `200`, `/contas` devolve `404` e `/api/stats` devolve `200`. A telemetria estruturada das últimas 48 horas, limitada aos seis paths v0 exatos, GET `2xx`, host de produção e excluindo `curl`, confirmou 69 pedidos ativos: `/api/stats` 31, `/api/portfolio` 1, `/api/recommendations` 1, `/api/outreach-followups` 16, `/api/email-followups` 9 e `/api/proposal-followups` 11, com tráfego até `2026-07-23T13:21:24Z`. As cópias privadas de owner review permanecem mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals. O PR `#1` permanece draft, mergeable e `CLEAN`, sem reviews, checks ou environments GitHub.

A implementação determinística até à Tarefa 18 continua verde. A primeira tarefa formalmente incompleta continua a ser a Tarefa 19, mas os gates anteriores também permanecem fechados: `parity=false`, 39 conflitos e uma Account/Lead em falta sem resolução ou aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL/backup automático off-host de produção ausentes, sem cutover/soak e sem dois releases estáveis sem consumidores v0. Retirar o legado agora quebraria tráfego observado. Produção, contratos legados e `.hermes/crm-revamp-complete.json` não foram alterados.

---

## Retoma autónoma em 2026-07-23T14:01:24Z

A execução partiu do `HEAD` limpo e sincronizado `b5381912192493da0c3cdf57d3ddfe6baacd3df8`, na branch esperada. O plano canónico, `CURRENT_STATE.md`, commits, suite, migrations, PR, staging, produção, backup, rollback, owner review, telemetria, processos outbound e sentinel foram reinspecionados. Não existia trabalho staged, unstaged ou untracked e os recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo em loopback, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 254,19 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda instância vazia exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. O backup off-host pós-shadow `crm-staging-20260723T090031Z-post-shadow.dump`, mode `0600`, 239.676 bytes e SHA-256 `a26eb502ca5a77422672d99ecd20a9d471fe074fbb3d78c2918a51110fac56ec`, foi restore-verificado numa terceira instância PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff no delta Python, `compileall`, `node --check` e `git diff --check` passaram. Os três recursos descartáveis foram removidos.

O staging continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL 16 estão `running`, com zero restarts e zero marcadores de erro desde `13:30Z`. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 66 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 158 Activities, zero Outbox rows e zero violações da invariante de Account. `/up` devolveu `200`, conteúdo protegido sem credenciais devolveu `401` e o Agent ingress desligado devolveu `404`. O timer diário de backup está `enabled`/`active`, com último resultado `success`; não existem workers, reconcilers ou publishers outbound.

Produção permanece inalterada no build pré-revamp: `/up=200`, `/contas=404` e `/api/stats=200`. A telemetria estruturada das últimas 48 horas, limitada aos seis paths v0 exatos, GET `2xx`, host de produção e excluindo `curl` e o probe `crm-gate-probe`, confirmou 79 pedidos ativos: `/api/stats` 41, `/api/outreach-followups` 20, `/api/email-followups` 8 e `/api/proposal-followups` 10, com tráfego até `2026-07-23T13:57:16Z`; `/api/portfolio` e `/api/recommendations` não tiveram tráfego não-probe na janela. As cópias privadas de owner review permanecem mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals. O PR `#1` aponta para o SHA exato da branch, permanece draft, `MERGEABLE/CLEAN`, sem reviews ou checks.

A implementação determinística até à Tarefa 18 continua verde, mas a conclusão global permanece falsa. Persistem `parity=false`, 39 conflitos e uma Account/Lead em falta sem resolução ou aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL CRM de produção com backup automático off-host e restore real ausentes, sem cutover/soak de produção e sem dois releases estáveis pós-cutover ou ausência comprovada de consumidores v0. Retirar o legado agora quebraria tráfego observado. Produção, contratos legados e `.hermes/crm-revamp-complete.json` não foram alterados.

---

## Retoma autónoma em 2026-07-23T14:58:46Z

A execução partiu do `HEAD` limpo e sincronizado `63807157c1d5a11acda9023e17052dc742c29a44`, na branch esperada. O plano canónico, este documento, commits, suite, migrations, PR, staging, produção, backup, owner review, processos outbound, cron autónomo e sentinel foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo em loopback, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 280,65 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. O container descartável foi removido. A migration de raiz `base -> 0013` passou antes da suite.

O staging continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL 16 estão `healthy`, com zero restarts. O timer diário de backup está `enabled`/`active`, com último resultado `success`, e não existem workers, reconcilers ou publishers outbound. O host tem cerca de 11 GB livres e 1,3 GB de memória disponível. `/up` de staging e produção devolveram `200`; produção continua pré-revamp, com `/contas=404` e `/api/stats=200`.

As cópias privadas de owner review permanecem mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals. O PR `#1` aponta para o SHA exato da branch, permanece draft e `MERGEABLE/CLEAN`. A implementação determinística até à Tarefa 18 continua verde, mas `parity=false`, 39 conflitos, uma Account/Lead em falta, owner review em `0/90`, decisões oficiais de `Won`/retenção/scopes, mapping e PostgreSQL/backup automático off-host de produção, cutover/soak e os dois releases estáveis sem consumidores v0 continuam ausentes. Produção, contratos legados e `.hermes/crm-revamp-complete.json` não foram alterados. O job autónomo `318572f3de7c` continua ativo para reavaliar os gates a partir de evidência real.

---

## Retoma autónoma em 2026-07-23T17:31:14Z

A execução partiu do `HEAD` limpo e sincronizado `cf8cb2e5a37a978601ca486756d73ec57cef6f51`, na branch esperada. O plano canónico, commits, suites, migrations, PR, staging, produção, backup, owner review, processos outbound, cron autónomo e sentinel foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo em loopback, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 394,76 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda base vazia exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. O backup off-host pós-shadow `crm-staging-20260723T090031Z-post-shadow.dump`, mode `0600`, 239.676 bytes e SHA-256 `a26eb502ca5a77422672d99ecd20a9d471fe074fbb3d78c2918a51110fac56ec`, foi restore-verificado numa terceira instância PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. `compileall`, oito `node --check` e `git diff --check` passaram. Os três recursos descartáveis foram removidos e as portas `56180`, `56181` e `56182` ficaram livres.

O staging continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL 16 estão `healthy`, com zero restarts. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 66 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 158 Activities, zero Outbox rows, 223 IngestEvents `applied` e zero violações da invariante de Account. Leads, Contas, Propostas, Inteligência, Operações e três APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`; o soak read-only executou `90/90` pedidos com sucesso, p95 `121,0 ms`, máximo `168,8 ms` e zero marcadores de erro. O timer de backup está `enabled`/`active`, com último resultado `success`, e não existem workers, reconcilers ou publishers outbound.

Produção permanece inalterada no build pré-revamp, sem PostgreSQL/backup CRM ou mapping canónico. `/up` devolve `200`, `/contas` devolve `404` e `/api/stats` devolve `200`. A janela atualmente retida pelo proxy não contém tráfego v0 não-probe, mas é mais curta do que o período exigido e a evidência do mesmo dia registou consumidores ativos; portanto não prova ausência de consumidores nem satisfaz os dois releases pós-cutover. As cópias privadas de owner review continuam mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals. O PR `#1` permanece draft, mergeable e sem reviews ou checks.

A implementação determinística até à Tarefa 18 permanece verde. A primeira tarefa formalmente incompleta continua a ser a Tarefa 19, mas os gates anteriores também permanecem fechados: `parity=false`, conflitos sem resolução/aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL/backup automático off-host de produção ausentes, sem cutover/soak e sem dois releases estáveis sem consumidores v0. Retirar o legado agora não tem evidência de segurança suficiente. Produção e `.hermes/crm-revamp-complete.json` não foram alterados.

---

## Retoma autónoma em 2026-07-23T18:28:06Z

A execução partiu do `HEAD` limpo e sincronizado `b6dc08337ad1c0521a3a4922084c508056518ccc`, na branch esperada. O plano canónico, este documento, commits, suites, migrations, PR, staging, produção, backup, owner review, telemetria, processos outbound, cron autónomo e sentinel foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; os muitos recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo em loopback, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 260,24 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda instância vazia exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. O backup off-host pós-shadow `crm-staging-20260723T090031Z-post-shadow.dump`, mode `0600`, 239.676 bytes e SHA-256 `a26eb502ca5a77422672d99ecd20a9d471fe074fbb3d78c2918a51110fac56ec`, foi restore-verificado em PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff no delta Python, `compileall`, oito `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 192 commits e cerca de 2,24 MB sem leaks. Os recursos descartáveis desta execução foram removidos e as portas `56200` e `56201` ficaram livres.

O staging no proxy/TLS final continua na imagem funcional `crm-staging:d061ce1`; o delta posterior até ao candidato é apenas documentação. Aplicação e PostgreSQL 16 estão `healthy`, com zero restarts e zero marcadores de erro nas duas horas observadas. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 66 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 158 Activities, zero Outbox rows, 223 IngestEvents `applied` e zero violações da invariante de Account. Leads, Contas, Propostas, Inteligência, Operações e três APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`; as superfícies protegidas públicas devolveram `401`. O timer diário de backup está `enabled`/`active`, com último resultado `success`, e não existem workers, reconcilers ou publishers outbound.

Produção permanece inalterada no build pré-revamp, sem configuração, mapping, PostgreSQL ou backup CRM. `/up` e o dashboard legado devolveram `200`; `/contas`, `/propostas` e `/inteligencia` devolveram `404`. A telemetria estruturada das últimas 48 horas, limitada aos seis paths v0 exatos, GET `2xx`, host de produção e excluindo `curl` e o probe `crm-gate-probe`, observou quatro pedidos ativos em `/api/stats`, o mais recente em `2026-07-23T18:00:53.887148865Z`. As cópias privadas de owner review permanecem mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals. O PR `#1` aponta para o SHA exato da branch, permanece draft e mergeable, sem reviews, checks, deployments, tags ou releases que contenham o candidato.

A implementação determinística até à Tarefa 18 continua verde, mas a conclusão global permanece falsa. Persistem `parity=false`, 39 conflitos e uma Account/Lead em falta sem resolução ou aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL CRM de produção com backup automático off-host e restore real ausentes, sem cutover/soak de produção e sem dois releases estáveis pós-cutover ou ausência comprovada de consumidores v0. A Tarefa 19 proíbe retirar contratos nestas condições. Produção, contratos legados e `.hermes/crm-revamp-complete.json` não foram alterados.

---

## Retoma autónoma em 2026-07-23T20:25:09Z

A execução partiu do `HEAD` limpo e sincronizado `101bc46bc37b86a2ae60c96975f678aa9d848d15`, na branch esperada. O plano canónico, este documento, commits, PR, staging, produção, backup, owner review, telemetria, processos outbound e sentinel foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; os muitos recursos locais preexistentes e desconhecidos foram preservados.

O PR `#1` aponta para o SHA exato da branch, permanece draft e mergeable, sem reviews ou checks. O staging continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL 16 estão `healthy`, com zero restarts. O timer diário de backup está `enabled`/`active`, a execução mais recente observada terminou com sucesso e a próxima execução está agendada. Não existem processos de worker CRM, reconciler, connector ou publisher outbound no host nem localmente. As cópias privadas de owner review continuam com `0/46` decisões de Accounts e `0/44` decisões de Proposals.

Produção permanece inalterada no build pré-revamp: `/up=200`, `/contas=404`, `/propostas=404`, `/inteligencia=404` e `/api/stats=200`. O host tem 11 GiB livres, mas produção continua sem configuração, mapping, PostgreSQL ou backup CRM. A telemetria estruturada das últimas 48 horas, limitada aos seis paths v0 exatos, GET `2xx`, host de produção e excluindo `curl` e `crm-gate-probe`, confirmou consumidores ativos: `/api/stats` 9, `/api/outreach-followups` 2, `/api/email-followups` 2 e `/api/proposal-followups` 2, com tráfego até `2026-07-23T20:00:53Z`.

Não existe uma ação técnica segura que feche os gates restantes sem fabricar dados, decisões ou tempo. Permanecem `parity=false`, conflitos sem resolução/aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL CRM de produção com backup automático off-host e restore real ausentes, sem cutover/soak e sem dois releases estáveis pós-cutover ou ausência comprovada de consumidores v0. Retirar o legado agora quebraria tráfego observado. Produção, contratos legados e `.hermes/crm-revamp-complete.json` não foram alterados.

---

## Retoma autónoma em 2026-07-23T21:23:55Z

A execução partiu do `HEAD` limpo e sincronizado `6266d7e56d3fa16d6d095c58d525592fa74ed983`, na branch esperada. O plano canónico, este documento, commits, suites, migrations, PR, staging, produção, backup, owner review, telemetria, processos outbound, cron autónomo e sentinel foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; os recursos locais preexistentes e desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo em loopback, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 239,79 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda base vazia exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. O backup off-host pós-shadow `crm-staging-20260723T090031Z-post-shadow.dump` foi restore-verificado numa terceira instância PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff nos ficheiros Python do delta, `compileall`, oito `node --check` e `git diff --check` passaram. Os recursos descartáveis foram removidos e as portas `56210`, `56211` e `56212` ficaram livres.

O staging no proxy/TLS final continua na imagem funcional `crm-staging:d061ce1`; o único delta até ao `HEAD` é este documento. A aplicação permanece `healthy`, com zero restarts, e o timer diário de backup está `enabled`/`active`, com último resultado `success`. `/up` devolveu `200` e `/leads` sem credenciais devolveu `401` com `no-store`. Não foram encontrados workers, reconcilers, connectors ou publishers outbound ativos; o único match inicial de processo era o próprio comando de inspeção.

Produção permanece inalterada no build pré-revamp: `/up=200`, `/contas=404` e `/api/stats=200`; continua sem mapping, PostgreSQL ou backup CRM. A telemetria estruturada das últimas 48 horas, limitada aos seis paths v0 exatos, GET `2xx`, host de produção e excluindo `curl` e `crm-gate-probe`, confirmou consumidores ativos: `/api/stats` 11, `/api/outreach-followups` 2, `/api/email-followups` 2 e `/api/proposal-followups` 2, com tráfego até `2026-07-23T21:00:53Z`. As cópias privadas de owner review permanecem mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals. O PR `#1` aponta para o SHA exato da branch, permanece draft e mergeable, sem reviews ou checks; não existem tags ou releases que contenham o candidato.

A implementação determinística até à Tarefa 18 permanece verde. A primeira tarefa formalmente incompleta continua a ser a Tarefa 19, mas os gates anteriores também permanecem fechados: `parity=false`, 39 conflitos e uma Account/Lead em falta sem resolução ou aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL/backup automático off-host de produção ausentes, sem cutover/soak de produção e sem dois releases estáveis pós-cutover ou ausência comprovada de consumidores v0. Retirar o legado agora quebraria tráfego observado. Produção, contratos legados e `.hermes/crm-revamp-complete.json` não foram alterados.

---

## Retoma autónoma em 2026-07-23T21:58:35Z

A execução partiu do `HEAD` limpo e sincronizado `a39b74510377d8b13d260002a363bd6265cb8eaf`, na branch esperada. O plano canónico, este documento, commits, PR, suite, migrations, staging, produção, backup, owner review, telemetria, processos outbound, cron autónomo e sentinel foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; todos os recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo em loopback, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 238,97 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda base vazia exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. O backup off-host pós-shadow `crm-staging-20260723T090031Z-post-shadow.dump`, mode `0600`, 239.676 bytes e SHA-256 `a26eb502ca5a77422672d99ecd20a9d471fe074fbb3d78c2918a51110fac56ec`, foi restore-verificado em PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff no delta Python, `compileall`, oito `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 195 commits e cerca de 2,25 MB sem leaks. Os dois PostgreSQL e ficheiros descartáveis desta execução foram removidos; as portas `56220` e `56221` ficaram livres.

O staging continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL 16 estão `healthy`, com zero restarts e zero marcadores de erro nas duas horas observadas. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 66 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 158 Activities, zero Outbox rows, 223 IngestEvents `applied`, zero eventos `failed`/`dead_letter` e zero violações da invariante de Account. Leads, Contas, Propostas, Inteligência, Operações e três APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`; as superfícies protegidas sem credenciais devolveram `401`. O timer diário de backup está `enabled`/`active`, com último resultado `success`, e não existem workers, reconcilers ou publishers outbound.

Produção permanece inalterada no build pré-revamp, sem configuração, mapping, PostgreSQL ou backup CRM. `/up=200`, `/contas=404`, `/propostas=404`, `/inteligencia=404` e `/api/stats=200`. A telemetria estruturada das últimas 48 horas, limitada aos seis paths v0 exatos, GET `2xx`, host de produção e excluindo `curl` e `crm-gate-probe`, confirmou consumidores ativos: `/api/stats` 11, `/api/outreach-followups` 2, `/api/email-followups` 2 e `/api/proposal-followups` 2, com tráfego até `2026-07-23T21:00:53Z`; `/api/portfolio` e `/api/recommendations` não tiveram tráfego elegível na janela. As cópias privadas de owner review permanecem mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals. O PR `#1` aponta para o SHA exato da branch, permanece draft e mergeable, sem reviews ou checks.

A implementação determinística até à Tarefa 18 continua verde, mas a conclusão global permanece falsa. Persistem `parity=false`, conflitos reais e uma Account/Lead em falta sem resolução ou aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL CRM de produção com backup automático off-host e restore real ausentes, sem cutover/soak de produção e sem dois releases estáveis pós-cutover ou ausência comprovada de consumidores v0. A Tarefa 19 proíbe retirar contratos nestas condições. Produção, contratos legados e `.hermes/crm-revamp-complete.json` não foram alterados.

---

## Retoma autónoma em 2026-07-24T00:31:15+01:00

A execução partiu do `HEAD` limpo e sincronizado `7975bac026346d4637b2c7462ac469feff93677b`, na branch esperada. O plano canónico, este documento, commits, PR, suites, migrations, staging, produção, backup, owner review, telemetria, processos outbound, cron autónomo e sentinel foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; os recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo em loopback, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 329,14 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda base vazia exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. Um dump custom-format dessa base foi restore-verificado em PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, zero workspaces e zero violações. Ruff nos ficheiros Python do delta, `compileall`, oito `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 196 commits e cerca de 2,25 MB sem leaks.

O staging no proxy/TLS final continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL 16 estão `healthy`, com zero restarts e zero marcadores de erro nas duas horas observadas. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 66 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 158 Activities, zero Outbox rows, 223 IngestEvents `applied`, zero eventos `failed`/`dead_letter` e zero violações da invariante de Account. Leads, Contas, Propostas, Inteligência, Operações e três APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`; conteúdo protegido sem credenciais devolveu `401`. O timer diário de backup está `active`, o último resultado foi `success` e não existem workers, reconcilers, connectors ou publishers outbound.

Produção permanece inalterada no build pré-revamp, sem configuração, mapping, PostgreSQL ou backup CRM. `/up=200`, `/contas=404` e `/api/stats=200`. A telemetria estruturada das últimas 48 horas, limitada aos seis paths v0 exatos, respostas `2xx`, host de produção e excluindo `curl` e `crm-gate-probe`, confirmou consumidores ativos: `/api/stats` 15, `/api/outreach-followups` 2, `/api/email-followups` 2 e `/api/proposal-followups` 2, com tráfego até `2026-07-23T23:00:52Z`; `/api/portfolio` e `/api/recommendations` não tiveram tráfego elegível na janela. As cópias privadas de owner review permanecem mode `0600`, com `0/46` decisões de Accounts e `0/44` decisões de Proposals. O PR `#1` aponta para o SHA exato da branch, permanece draft e mergeable, sem reviews ou checks.

Não existe uma ação técnica segura que feche os gates restantes sem fabricar dados, decisões ou tempo. Permanecem `parity=false`, conflitos sem resolução/aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL CRM de produção com backup automático off-host e restore real ausentes, sem cutover/soak e sem dois releases estáveis pós-cutover ou ausência comprovada de consumidores v0. Retirar o legado agora quebraria tráfego observado. Produção, contratos legados e `.hermes/crm-revamp-complete.json` não foram alterados.

---

## Verificação autoritativa desta retoma em 2026-07-24T00:32:51Z

O trabalho staged herdado em `CURRENT_STATE.md` foi preservado. Num PostgreSQL 16 descartável exclusivo em loopback, migrado de raiz até `0013`, a suite completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência e exit `0`; os quatro módulos frontend passaram `37/37`. Numa segunda base vazia, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`, `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. O backup off-host pós-shadow `crm-staging-20260723T090031Z-post-shadow.dump`, mode `0600`, 239.676 bytes e SHA-256 `a26eb502ca5a77422672d99ecd20a9d471fe074fbb3d78c2918a51110fac56ec`, foi restore-verificado num terceiro PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff no delta Python, `compileall`, oito `node --check`, `git diff --check` e Gitleaks passaram; os recursos descartáveis desta retoma foram removidos.

O staging continua em `crm-staging:d061ce1`, único candidato funcional do branch; o delta posterior é apenas este documento. Aplicação e PostgreSQL 16 estão `healthy`, com zero restarts; o timer de backup está `enabled`/`active`, com último resultado `success`, e não existem workers, reconcilers, connectors ou publishers outbound. A base mantém schema `0013`, um workspace, 46 Accounts, 46 Contacts, 66 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 158 Activities, zero Outbox rows, 223 IngestEvents `applied`, zero eventos `failed`/`dead_letter` e zero violações da invariante de Account. Leads, Contas, Propostas, Inteligência, Operações e três APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`; sem credenciais devolveram `401`.

Produção permanece inalterada no build pré-revamp: `/up=200`, `/contas=404`, `/propostas=404`, `/inteligencia=404` e `/api/stats=200`. A telemetria estruturada das últimas 48 horas, limitada aos seis paths v0 exatos, GET `2xx`, host de produção e excluindo `curl` e `crm-gate-probe`, confirmou consumidores ativos: `/api/stats` 17, `/api/outreach-followups` 2, `/api/email-followups` 2 e `/api/proposal-followups` 2, com tráfego até `2026-07-24T00:00:53Z`; `/api/portfolio` e `/api/recommendations` não tiveram tráfego elegível. As cópias privadas de owner review continuam mode `0600`, com `0/46` decisões de Accounts e `0/44` de Proposals. A conclusão global, a produção, os contratos legados e o sentinel permanecem corretamente bloqueados pelos gates de dados, owner acceptance, cutover e release definidos no plano.

---

## Retoma autónoma em 2026-07-24T03:31:06Z

A execução partiu do `HEAD` limpo e sincronizado `4629432296a89a3fceb17dae493fcc0b6e5f0e59`, na branch esperada. O plano canónico, este documento, commits, PR, suites, migrations, staging, produção, backup, owner review, processos outbound, cron autónomo e sentinel foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; todos os recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo em loopback, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 243,60 segundos e exit explícito `0`; os quatro módulos frontend Node passaram `37/37`. Numa segunda base vazia exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. Ruff nos ficheiros Python do delta, `compileall`, oito `node --check` e `git diff --check` passaram. Os dois PostgreSQL descartáveis foram removidos e as portas `56230` e `56231` ficaram livres.

O staging no proxy/TLS final continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL 16 estão `healthy`, com zero restarts. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 66 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 158 Activities, zero Outbox rows, 223 IngestEvents `applied` e zero violações da invariante de Account. `/up` devolveu `200`; Leads, Contas, Propostas, Inteligência, Operações e três APIs v1 devolveram `200` autenticado; as superfícies protegidas sem credenciais devolveram `401`. O timer diário de backup está `enabled`/`active`, com último resultado `success`, e não existem workers, reconcilers ou publishers outbound.

Produção permanece inalterada no build pré-revamp, com `/up=200`, `/contas=404`, `/propostas=404`, `/inteligencia=404` e `/api/stats=200`. A telemetria estruturada das últimas 48 horas, limitada aos seis paths v0 exatos, GET `2xx`, host de produção e excluindo `curl` e `crm-gate-probe`, confirmou consumidores ativos: `/api/stats` 26, `/api/outreach-followups` 2, `/api/email-followups` 2 e `/api/proposal-followups` 2, com tráfego até `2026-07-24T03:00:52Z`; `/api/portfolio` e `/api/recommendations` não tiveram tráfego elegível. O PR `#1` aponta para o SHA exato da branch, permanece draft, mergeable e `CLEAN`, sem reviews ou checks. As cópias privadas de owner review continuam mode `0600`, com `0/46` decisões de Accounts e `0/44` de Proposals.

A implementação determinística até à Tarefa 18 permanece verde, mas a conclusão global continua falsa. Persistem `parity=false`, conflitos reais e uma Account/Lead em falta sem resolução ou aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL CRM de produção com backup automático off-host e restore real ausentes, sem cutover/soak de produção e sem dois releases estáveis pós-cutover ou ausência comprovada de consumidores v0. Estes gates não podem ser fabricados por código ou autorização autónoma. Produção, contratos legados e `.hermes/crm-revamp-complete.json` não foram alterados.

---

## Retoma autónoma em 2026-07-24T06:00:52+01:00

A execução partiu do `HEAD` limpo e sincronizado `894938404819db8a28f7716d76b65dabb151acc0`, na branch esperada. O plano canónico, este documento, commits, PR, suite, migrations, staging, produção, backup, owner review, telemetria, processos outbound, cron autónomo e sentinel foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; os recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo em loopback, explicitamente marcado para testes e migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 253,21 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda base vazia exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. O backup off-host pós-shadow `crm-staging-20260723T090031Z-post-shadow.dump`, mode `0600`, 239.676 bytes, foi restore-verificado em PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff nos ficheiros Python do delta, `compileall`, oito `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 198 commits e cerca de 2,26 MB sem leaks. O container e as bases descartáveis foram removidos e a porta `56240` ficou livre.

O staging no proxy/TLS final continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL 16 estão `healthy`, com zero restarts. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 66 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 158 Activities, zero Outbox rows, zero eventos `failed`/`dead_letter` e zero violações da invariante de Account. Leads, Contas, Propostas, Inteligência, Operações e três APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`; sem credenciais o staging devolveu `401`. O timer diário de backup está `enabled`/`active`, com último resultado `success`, e não existem workers, reconcilers, connectors ou publishers outbound.

Produção permanece inalterada no build pré-revamp: `/up=200`, `/contas=404`, `/propostas=404`, `/inteligencia=404` e `/api/stats=200`; continua sem mapping, PostgreSQL ou backup CRM. A telemetria estruturada das últimas 48 horas, limitada aos seis paths v0 exatos, GET `2xx`, host de produção e excluindo `curl` e `crm-gate-probe`, confirmou consumidores ativos: `/api/stats` 29, `/api/outreach-followups` 2, `/api/email-followups` 2 e `/api/proposal-followups` 2, com tráfego até `2026-07-24T04:54:50Z`. As cópias privadas de owner review permanecem mode `0600`, com `0/46` decisões de Accounts e `0/44` de Proposals. O PR `#1` aponta para o SHA inicial exato da retoma, permanece draft e mergeable, sem reviews ou checks.

A primeira tarefa formalmente incompleta continua a ser a Tarefa 19, mas os gates anteriores também permanecem fechados: `parity=false`, conflitos sem resolução/aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL/backup automático off-host de produção ausentes, sem cutover/soak e sem dois releases estáveis sem consumidores v0. Retirar o legado agora quebraria tráfego observado. Produção, contratos legados e `.hermes/crm-revamp-complete.json` não foram alterados.

---

## Retoma autónoma em 2026-07-24T05:36:12Z

A execução partiu do `HEAD` limpo e sincronizado `ca1eaca85c4ff1dae6c0ab9a1c7bfd244ff7338e`, na branch esperada. O plano canónico, este documento, commits, PR, suites, migrations, staging, produção, backup, owner review, telemetria, processos outbound, cron autónomo e sentinel foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; os recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo em loopback, explicitamente marcado para testes e migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 336,63 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda base vazia exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)` e `alembic check` não detetou operações pendentes. O backup off-host pós-shadow `crm-staging-20260723T090031Z-post-shadow.dump`, mode `0600`, 239.676 bytes e SHA-256 `a26eb502ca5a77422672d99ecd20a9d471fe074fbb3d78c2918a51110fac56ec`, foi restore-verificado em PostgreSQL 16 com schema `0013`, 15 tabelas obrigatórias, um workspace e zero violações. Ruff no delta Python, `compileall`, oito `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 199 commits e cerca de 2,27 MB sem leaks. Os recursos descartáveis desta retoma foram removidos e as portas `56250` a `56253` ficaram livres.

O staging no proxy/TLS final continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL 16 estão `healthy`, com zero restarts. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 66 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 158 Activities, zero Outbox rows, 223 IngestEvents `applied` e zero violações da invariante de Account. Leads, Contas, Propostas, Inteligência, Operações e três APIs v1 devolveram `200` autenticado com `Cache-Control: no-store`; sem credenciais, `/leads` devolveu `401`. O timer diário de backup está `enabled`/`active`, com último resultado `success`, e não existem workers, reconcilers, connectors ou publishers outbound.

Produção permanece inalterada no build pré-revamp `7622a2b2b8d5e0790858208b2c3a1f119edb7328`, sem configuração, mapping, PostgreSQL ou backup CRM. `/up=200`, `/contas=404`, `/propostas=404`, `/inteligencia=404` e `/api/stats=200`. A janela atualmente retida pelo proxy contém apenas um pedido aos seis paths v0 acompanhados, identificado como `curl`; esta janela curta não prova ausência de consumidores, porque a evidência do mesmo dia registada acima observou tráfego não-probe ativo e ainda não existem dois releases pós-cutover. As cópias privadas de owner review continuam mode `0600`, com `0/46` decisões de Accounts e `0/44` de Proposals. O PR `#1` aponta para o SHA exato inicial desta retoma, permanece draft e mergeable, sem reviews ou checks.

A implementação determinística até à Tarefa 18 continua verde, mas a conclusão global permanece falsa. Persistem `parity=false`, 39 conflitos e uma Account/Lead em falta sem resolução ou aceitação, owner review em `0/90`, políticas oficiais de `Won`/retenção/scopes por decidir, mapping e PostgreSQL CRM de produção com backup automático off-host e restore real ausentes, sem cutover/soak de produção e sem dois releases estáveis pós-cutover ou ausência comprovada de consumidores v0. Estes gates de dados, negócio, produção e tempo não podem ser fabricados pela execução autónoma. Produção, contratos legados e `.hermes/crm-revamp-complete.json` não foram alterados.

---

## Retoma autónoma em 2026-07-24T10:05:32Z

A execução partiu do `HEAD` limpo e sincronizado `75212d1c0ab77808e278bddfbcf8ee52c162b678`, na branch esperada. O plano canónico, este documento, commits, PR, suites, migrations, staging, produção, backup, owner review, processos outbound, cron autónomo e sentinel foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; os recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo em loopback, explicitamente marcado para testes e migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 462,15 segundos e exit explícito `0`. Os quatro módulos frontend Node passaram `37/37`. Numa segunda base vazia exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)`, `alembic check` não detetou operações pendentes e um dump custom-format restaurou com schema `0013`, 15 tabelas obrigatórias, zero workspaces e zero violações. Ruff no delta Python, `compileall`, oito `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 200 commits e cerca de 2,27 MB sem leaks. Os recursos descartáveis desta retoma foram removidos e as portas `56270` e `56271` ficaram livres.

O staging no proxy/TLS final continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL 16 estão `healthy`, com zero restarts. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 66 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 158 Activities, zero Outbox rows, 223 IngestEvents `applied` e zero violações da invariante de Account. Leads, Contas, Propostas, Inteligência, Operações e três APIs v1 devolveram `200` autenticado; sem credenciais, `/leads` devolveu `401`. O timer diário de backup está `enabled`/`active`, com último resultado `success`, e não existem workers, reconcilers ou publishers outbound. As cópias privadas de owner review continuam mode `0600`, com `0/46` decisões de Accounts e `0/44` de Proposals.

Produção permanece inalterada no build pré-revamp, sem PostgreSQL/backup CRM ou mapping canónico: `/up=200`, `/contas=404` e `/api/stats=200`. O PR `#1` aponta para o SHA exato inicial desta retoma, permanece draft e mergeable, sem reviews ou checks; não existem releases ou tags que contenham o candidato. A implementação determinística até à Tarefa 18 continua verde, mas os gates de dados, owner acceptance, produção, cutover, soak e dois releases estáveis continuam fechados. A Tarefa 19, o cutover e `.hermes/crm-revamp-complete.json` permanecem corretamente bloqueados.

---

## Retoma autónoma em 2026-07-24T10:35:11Z

A execução partiu do `HEAD` limpo e sincronizado `fbdbd7c7d2b198b5c56ffa5c88b90943ea4d627b`, na branch esperada. O plano canónico, `CURRENT_STATE.md`, commits, PR, suites, migrations, staging, produção, backup, owner review, telemetria, processos outbound, cron autónomo e sentinel foram reinspecionados. Não existia trabalho staged, unstaged ou untracked; todos os recursos preexistentes desconhecidos foram preservados.

Num PostgreSQL 16 descartável exclusivo em loopback, migrado de raiz até `0013`, a suite Python completa passou com `1202 passed, 1 skipped`, uma `StarletteDeprecationWarning` da dependência, 278,08 segundos e exit explícito `0`; os quatro módulos frontend Node passaram `37/37`. Numa segunda base vazia exclusiva, o lifecycle passou `base -> 0013 -> 0011 -> 0013 -> base -> 0013`; `alembic current` devolveu `0013 (head)`, `alembic check` não detetou operações pendentes e um dump custom-format foi restore-verificado com schema `0013`, 15 tabelas obrigatórias, zero workspaces e zero violações. Ruff no delta Python, `compileall`, oito `node --check`, `git diff --check` e Gitleaks passaram; Gitleaks analisou 201 commits e cerca de 2,27 MB sem leaks. Os recursos descartáveis desta retoma foram removidos e as portas `56300` e `56310` ficaram livres.

O staging no proxy/TLS final continua na imagem funcional `crm-staging:d061ce1`; aplicação e PostgreSQL 16 estão `healthy`, com zero restarts. A consulta agregada confirmou schema `0013`, um workspace, 46 Accounts, 46 Contacts, 66 Leads, 44 Proposals, 44 ProposalVersions, 21 Tasks, 158 Activities, zero Outbox rows, 223 IngestEvents `applied` e zero violações da invariante de Account. O timer diário de backup está `enabled`/`active`, com último resultado `success`, e não existem workers, reconcilers, connectors ou publishers outbound. As cópias privadas de owner review permanecem mode `0600`, com `0/46` decisões de Accounts e `0/44` de Proposals.

Produção permanece inalterada no build pré-revamp: `/up=200`, `/contas=404` e `/api/stats=200`; continua sem mapping, PostgreSQL ou backup CRM. Na janela estruturada efetivamente retida pelo proxy, entre `2026-07-24T05:16:17Z` e `2026-07-24T10:34:05Z`, limitada aos seis paths v0 exatos, GET `2xx`, host de produção e excluindo `curl` e `crm-gate-probe`, foram observados dez pedidos em `/api/stats` e um em `/api/outreach-followups`, com tráfego até `2026-07-24T10:00:52Z`. O PR `#1` aponta para o SHA inicial exato da retoma, permanece draft, mergeable e sem checks ou reviews.
