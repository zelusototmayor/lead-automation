...
As of 2026-07-21, migrations through `0013`, backfills, connectors, restore tooling, guarded cutover flags, the scoped browser identity adapter and the canonical operational workspace have passed the safe suite, PostgreSQL lifecycle, real-Sheet replay, backup restore and local/container smoke gates.

A persistent, logically isolated staging environment is now running at the final TLS/proxy hostname on the shared droplet. PostgreSQL 16 uses a dedicated container, volume and network; the application image is built from the exact functional candidate recorded in `CURRENT_STATE.md`. Contas and Propostas read PostgreSQL and the staging-only human command writer uses PostgreSQL; Sheets projection, Agent ingress, connectors, workers and outbox publishing remain disabled. The real snapshot was applied twice: the second account and proposal backfills created zero new records. Authenticated HTTP and browser smoke pass, unauthenticated rich routes return `401`, `/up` remains public, and an exact staging backup has been copied off-host with mode `0600` and successfully restore-verified on a separate PostgreSQL 16 instance.
