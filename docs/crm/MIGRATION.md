...
As of 2026-07-17, migrations through `0006`, backfills, connectors, restore tooling, guarded cutover flags and the scoped rich-route identity adapter have been exercised against disposable local PostgreSQL. The relevant CRM suite passes locally. A read-only real-Sheet shadow run proved replay idempotency for applicable rows but still has unresolved identity/account conflicts. The observed live dashboard still runs commit/image `7622a2b` without PostgreSQL; no staging environment, production principal mapping, owner-approved sample, soak or production cutover has been verified from this branch. Aggregate proxy telemetry also shows active v0 API consumers, so Task 19 retirement is blocked independently of the cutover gates. These are blocking gates, not optional follow-up work.
