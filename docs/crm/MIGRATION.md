...
As of 2026-07-18, migrations through `0007`, backfills, connectors, restore tooling, guarded cutover flags and the scoped browser-read identity adapter have been exercised against disposable local PostgreSQL and an isolated ephemeral GitHub Codespaces staging rehearsal at the exact feature-branch SHA. Migration `0007` adds canonical email, meeting, task and reconciliation records; account reads now derive communication and meeting metrics from those canonical facts, and runtime-mode/retry tests cover auth-before-I/O, legacy dependency isolation and recoverable ingestion failures. The safe suite, PostgreSQL lifecycle, real-Sheet replay, backup restore, HTTP/browser smoke and a short technical soak passed in that isolated rehearsal. The real-data comparison still has unresolved identity/account conflicts, and the Codespace is not a substitute for the final TLS/proxy environment, production-grade automatic backups, an owner-approved sample or the required production soak. The observed live dashboard still serves the pre-revamp build without the new routes; no production CRM database, cutover or post-cutover release has been verified from this branch. Aggregate proxy telemetry previously showed active v0 API consumers, so Task 19 retirement remains blocked independently of the cutover gates. These are blocking gates, not optional follow-up work.
