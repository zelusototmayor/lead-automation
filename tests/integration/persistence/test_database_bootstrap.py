...
def test_deployment_secret_template_covers_all_required_secrets() -> None:
    deploy_config = yaml.safe_load((REPO_ROOT / "dashboard/config/deploy.yml").read_text())
    required_secrets = set(deploy_config["env"]["secret"])
    assignments = {
        line.split("=", 1)[0]: line.split("=", 1)[1]
        for line in (REPO_ROOT / "dashboard/.kamal/secrets.example").read_text().splitlines()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param("postgresql+psycopg://localhost:65536/crm_test", id="port-too-large"),
        ...
        pytest.param("postgresql+psycopg://bad%20host/crm_test", id="encoded-host-whitespace"),
        pytest.param("postgresql+psycopg://999.999.999.999/crm_test", id="out-of-range-ipv4"),
        pytest.param(
        ...
        pytest.param("postgresql+psycopg://-bad.example/crm_test", id="leading-label-hyphen"),
        pytest.param("postgresql+psycopg://bad-.example/crm_test", id="trailing-label-hyphen"),
        pytest.param(
        ...
        pytest.param("postgresql+psycopg://localhost/crm test", id="database-whitespace"),
        pytest.param("postgresql+psycopg://localhost/crm\ntest", id="database-control"),
        pytest.param("postgresql+psycopg://localhost/crm%00test", id="encoded-database-control"),
    ],
)
...
def test_database_settings_reject_libpq_identity_query_overrides_without_leaking_input(
    ...
    value = f"postgresql+psycopg://secret-user:secret-password@localhost/crm_test?{query}"
    monkeypatch.setenv("DATABASE_URL", value)

    with pytest.raises(ValidationError) as exc_info:
        DatabaseSettings()

    rendered_errors = (str(exc_info.value), repr(exc_info.value.errors()), exc_info.value.json())
    for rendered_error in rendered_errors:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param("postgresql+psycopg://xn--mnchen-3ya.example/crm_test", id="punycode-dns"),
        pytest.param("postgresql+psycopg://münchen.example/crm_test", id="unicode-idna-dns"),
        pytest.param("postgresql+psycopg://db.example.com./crm_test", id="trailing-root-dot"),
        pytest.param(
...
def test_database_settings_accept_supported_network_hosts(
    ...
    for serialized in (repr(settings), repr(settings.model_dump()), settings.model_dump_json()):
...
def test_alembic_bootstrap_and_session_factory_work_from_any_cwd() -> None:
    ...
    commands = [
        ([sys.executable, "-m", "alembic", "-c", str(ALEMBIC_CONFIG), "upgrade", "head"], REPO_ROOT),
        ([sys.executable, "-m", "alembic", "-c", str(ALEMBIC_CONFIG), "upgrade", "head"], REPO_ROOT.parent),
    ]
