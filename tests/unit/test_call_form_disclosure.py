...
def test_call_dimensions_are_closed_by_default_without_removing_capture():
    ...
    assert p.found
    assert 'outcome_code' in p.fields
    assert 'name="outcome_code" value="connected" required' not in source
    assert source.index('id="call-note"') < source.index('data-call-advanced')
