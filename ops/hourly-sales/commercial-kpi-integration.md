...
The commands use the current Lisbon anchor date and the same monthly inventory/full historical-context routine. Callable `run_reconciliation(..., anchor_date=..., entrypoint=..., classification_only=True)` is exercised by the candidate tests. Both entrypoints share the same checkpoint/lease. Do not run them concurrently deliberately in production or guess/reset a checkpoint. Cursor signatures are process-local: a server restart/another worker can invalidate a cursor. The client automatically restarts the full read at most twice (three sweeps total) on source-cursor HTTP400 or HTTP409, preserving the SAME run/cutoff and all committed assessments. Each sweep re-reads current source/context and exact receipts before acknowledging; it never advances over an unread page or reclassifies a still-current prefix. Other auth/validation/rate/transport failures are not retried by this recovery. A competing active lease remains authoritative, including across client interruption; a new client invocation can proceed after lease expiry and reuse the committed prefix. Repeated multi-worker cursor churn fails bounded and leaves the checkpoint unchanged: use the reviewed single-worker/sticky route at release, do not configure a new signing secret or bypass lease protection. On failure inspect the bounded error and pending counts, then retry through the same interface. Do not claim success from a configured schedule or a process starting.

Late transactions with created_at before cutoff invalidate the current page manifest and force a re-read. Records created after cutoff (including backdated occurrences) are picked up by the next mandatory full monthly sweep, which is never filtered only above the old watermark. Their history can invalidate existing classifications immediately; finish cannot certify stale readbacks. Raw source/history and existing AuditEvent/SyncCheckpoint schema are unchanged.

The protected source payload adds company_display_name (canonical merged account display_name, then lead company_name fallback; null means unidentified). UI presents company and event time in Europe/Lisbon plus reason in the protected list, with original note/correction behind Abrir fonte. The public aggregate contains no company name or new source identifier. generated_at is labelled as the data read/update time, distinct from assessed_at (oldest current assessment); stale/pending remain explicit, never certified by a new GET timestamp.
